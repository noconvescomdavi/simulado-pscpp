...
function vectorAssetSvg(asset){if(!asset)return null;const ns="http://www.w3.org/2000/svg",svg=document.createElementNS(ns,"svg");svg.setAttribute("viewBox",asset.viewBox||`0 0 ${asset.width||800} ${asset.height||600}`);svg.setAttribute("preserveAspectRatio","xMidYMid meet");svg.setAttribute("aria-hidden","true");svg.style.cssText="display:block;width:100%;height:100%;overflow:visible";const byId=Object.fromEntries((asset.paths||[]).map(p=>[p.id,p]));const make=p=>{const path=document.createElementNS(ns,"path");path.setAttribute("d",p.d||"");path.setAttribute("fill",p.fill||"none");path.setAttribute("stroke",p.stroke||"none");path.setAttribute("stroke-width",String(p.strokeWidth??2));path.setAttribute("opacity",String(p.opacity??1));path.setAttribute("fill-rule",p.fillRule||"nonzero");path.setAttribute("vector-effect","non-scaling-stroke");return path};for(const p of asset.paths||[]){if(p.visible===false)continue;if(p.boolean?.op&&Array.isArray(p.boolean.sources)){const sources=p.boolean.sources.map(id=>byId[id]).filter(Boolean);if(!sources.length)continue;if(p.boolean.op==="union"||p.boolean.op==="exclude"){svg.appendChild(make({...p,d:sources.map(x=>x.d||"").join(" "),fillRule:p.boolean.op==="exclude"?"evenodd":(p.fillRule||"nonzero")}));continue}if(p.boolean.op==="intersect"&&sources.length>=2){const defs=svg.querySelector("defs")||svg.insertBefore(document.createElementNS(ns,"defs"),svg.firstChild),clip=document.createElementNS(ns,"clipPath"),cid="clip_"+String(p.id||Math.random()).replace(/[^a-z0-9_-]/gi,"");clip.id=cid;clip.appendChild(make({...sources[1],fill:"#fff",stroke:"none"}));defs.appendChild(clip);const q=make({...sources[0],fill:p.fill||sources[0].fill,stroke:p.stroke||sources[0].stroke});q.setAttribute("clip-path",`url(#${cid})`);svg.appendChild(q);continue}}svg.appendChild(make(p))}return svg}
