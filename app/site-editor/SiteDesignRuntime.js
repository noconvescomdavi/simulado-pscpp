...
function vectorAssetSvg(asset){if(!asset)return null;const svg=document.createElementNS("http://www.w3.org/2000/svg","svg");svg.setAttribute("viewBox",asset.viewBox||`0 0 ${asset.width||800} ${asset.height||600}`);svg.setAttribute("preserveAspectRatio","xMidYMid meet");svg.setAttribute("aria-hidden","true");svg.style.cssText="display:block;width:100%;height:100%;overflow:visible";for(const p of asset.paths||[]){const path=document.createElementNS("http://www.w3.org/2000/svg","path");path.setAttribute("d",p.d||"");path.setAttribute("fill",p.fill||"none");path.setAttribute("stroke",p.stroke||"none");path.setAttribute("stroke-width",String(p.strokeWidth??2));path.setAttribute("opacity",String(p.opacity??1));path.setAttribute("fill-rule",p.fillRule||"nonzero");path.setAttribute("vector-effect","non-scaling-stroke");svg.appendChild(path)}return svg}
