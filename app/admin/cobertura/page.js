...
export default function Page(){
 ...
 const critical=chapters.filter(x=>x.count===0).length,low=chapters.filter(x=>x.count>0&&x.count<25).length,target=chapters.filter(x=>x.count>=25&&x.count<=50).length,wide=chapters.filter(x=>x.count>50).length;
 ...
 return <main className="wrap admin-wrap coverage">
   ...
   <div className="coverage-legend"><span data-s="critical">0 crítico</span><span data-s="low">1–24 insuficiente</span><span data-s="target">25–50 meta</span><span data-s="wide">&gt;50 ampla</span></div>
