...
export default function EditorClient(){
  ...
  function addIconAsset(icon){const id="blk_icon_"+Math.random().toString(36).slice(2,8);const block={id,type:"icon",title:icon.name,iconCategory:icon.c||"",svgPath:icon.path,style:{width:"64px",height:"64px",color:"#0b3b5b"}};remember();setDesign(prev=>{const n=clone(prev);n.pages||={};n.pages[page]||={elements:{},blocks:[],settings:{}};n.pages[page].blocks=[...(n.pages[page].blocks||[]),block];return n});setSelectedBlockId(id);setTarget(null);setStudioMenuOpen(false);setStatus("Ícone adicionado ao canvas.")}
  function addLogoAsset(logo){const id="blk_logo_"+Math.random().toString(36).slice(2,8);const block={id,type:"logo",title:logo.wordmark||"ESTIBORDO",tagline:logo.tagline||"",svgPath:logo.mark||"",logoShape:logo.shape||"none",style:{display:"inline-flex",alignItems:"center",gap:"12px",color:"#0b3b5b",paddingTop:"10px",paddingRight:"14px",paddingBottom:"10px",paddingLeft:"14px",borderRadius:logo.shape==="rounded"?"16px":logo.shape==="circle"?"999px":"12px"}};remember();setDesign(prev=>{const n=clone(prev);n.pages||={};n.pages[page]||={elements:{},blocks:[],settings:{}};n.pages[page].blocks=[...(n.pages[page].blocks||[]),block];return n});setSelectedBlockId(id);setTarget(null);setStudioMenuOpen(false);setStatus("Logo vetorial adicionado. Edite texto, tagline e estilo no painel.")}
