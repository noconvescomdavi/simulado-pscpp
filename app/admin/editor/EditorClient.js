...
export default function EditorClient(){
  ...
  function blockComponents(){return design?.global?.blockComponents||[]}
  function createBlockMaster(){
    if(!selectedBlock)return;const name=window.prompt("Nome do componente mestre:",selectedBlock.title||selectedBlock.type);if(!name)return;
    const id="master_"+Math.random().toString(36).slice(2,8);remember();setDesign(prev=>{const n=clone(prev);n.global||={};n.global.blockComponents=[...(n.global.blockComponents||[]),{id,name,snapshot:clone(selectedBlock),updatedAt:new Date().toISOString()}];n.pages[page].blocks=updateBlockTree(n.pages[page].blocks||[],selectedBlockId,b=>({...b,componentMasterId:id,componentName:name}));return n});setStatus("Componente mestre criado. Novas instâncias podem reutilizar este símbolo.");
  }
  function updateBlockMaster(){
    if(!selectedBlock?.componentMasterId){setStatus("Este bloco não é um componente mestre.");return}
    remember();setDesign(prev=>{const n=clone(prev);n.global||={};n.global.blockComponents=(n.global.blockComponents||[]).map(c=>c.id===selectedBlock.componentMasterId?{...c,snapshot:clone(selectedBlock),updatedAt:new Date().toISOString()}:c);return n});setStatus("Mestre atualizado; instâncias serão sincronizadas no runtime.");
  }
  function createBlockInstance(masterId){
    const master=blockComponents().find(x=>x.id===masterId);if(!master)return;const instance={id:"blk_instance_"+Math.random().toString(36).slice(2,8),type:"componentInstance",componentRef:masterId,componentName:master.name,variant:"default",style:{}};remember();setDesign(prev=>{const n=clone(prev);n.pages[page].blocks=[...(n.pages[page].blocks||[]),instance];return n});setSelectedBlockId(instance.id);setStatus("Instância do componente adicionada.");
  }
  function compareDraft(){
    const before=original||{},after=design||{};let changed=0,added=0,removed=0;
    const walk=(a,b)=>{const ka=a&&typeof a==="object"?Object.keys(a):[],kb=b&&typeof b==="object"?Object.keys(b):[];for(const k of new Set([...ka,...kb])){if(!(k in (a||{}))){added++;continue}if(!(k in (b||{}))){removed++;continue}const av=a[k],bv=b[k];if(av&&bv&&typeof av==="object"&&typeof bv==="object")walk(av,bv);else if(JSON.stringify(av)!==JSON.stringify(bv))changed++}};
    walk(before,after);const summary={changed,added,removed,total:changed+added+removed};setDiffSummary(summary);setProPanelOpen(true);setStatus("Comparação concluída.");
  }
  function openDraftPreview(){
    try{localStorage.setItem("estibordo-preview:"+page,JSON.stringify(design));const url=page+(page.includes("?")?"&":"?")+"estibordoDraft=1";window.open(url,"_blank","noopener,noreferrer");setStatus("Preview de rascunho aberto em nova guia.")}catch{setStatus("Não foi possível abrir preview de rascunho.")}
  }
  ...
  return <main className="ev-app">
    <header className="ev-topbar">
      ...
      <div className="ev-toolbar">
        ...
        <button className={panMode?"is-active":""} onClick={()=>setPanMode(v=>!v)} title="Pan / canvas infinito">✋ Pan</button>
    ...
    <div className="ev-workspace">
      ...
      <section className="ev-canvas">
        ...
        <div className={"ev-frame-area "+(panMode?"is-panning":"")} ref={frameAreaRef}
          onPointerDown={e=>{if(!panMode)return;panRef.current={x:e.clientX,y:e.clientY,l:e.currentTarget.scrollLeft,t:e.currentTarget.scrollTop};e.currentTarget.setPointerCapture?.(e.pointerId)}}
          onPointerMove={e=>{if(!panMode||!panRef.current)return;e.currentTarget.scrollLeft=panRef.current.l-(e.clientX-panRef.current.x);e.currentTarget.scrollTop=panRef.current.t-(e.clientY-panRef.current.y)}}
          onPointerUp={e=>{panRef.current=null;e.currentTarget.releasePointerCapture?.(e.pointerId)}}>
