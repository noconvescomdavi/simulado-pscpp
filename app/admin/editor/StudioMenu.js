...
export default function StudioMenu({open,onClose,replacementMode,replacementType,selectionMode,onSelectionMode,siteMap,page,onPageChange,layers,onSelectLayer,blocks,selectedBlockId,onSelectBlock,onOpenBuilder,onOpenFlashcards,media,onUploadMedia,onAddIcon,onAddLogo,onAddSignalFlag,onImportImage,customCode,onCustomCode,breakpoints,onBreakpoints,onAddBreakpoint,onRemoveBreakpoint,blockComponents,onCreateInstance}){
  ...
  {tab==="tools"&&<><div className="ev-studio-tool-grid"><button onClick={onOpenBuilder}><i>＋</i><b>Page Builder</b><span>Blocos, templates e layouts</span></button><button onClick={onOpenFlashcards}><i>▣</i><b>Flashcards</b><span>Gerenciador acadêmico</span></button></div><section><h4>Responsividade / Breakpoints</h4><div className="ev-breakpoint-editor"><label><span>Tablet até</span><input type="number" value={breakpoints.tablet||1024} onChange={e=>onBreakpoints({tablet:Number(e.target.value)||1024})}/><small>px</small></label><label><span>Mobile até</span><input type="number" value={breakpoints.mobile||620} onChange={e=>onBreakpoints({mobile:Number(e.target.value)||620})}/><small>px</small></label></div><div className="ev-custom-bp-list">{(breakpoints.custom||[]).map(bp=><div key={bp.id}><b>{bp.label}</b><span>{bp.maxWidth}px</span><button onClick={()=>onRemoveBreakpoint(bp.id)}>×</button></div>)}</div><button className="ev-import-asset" onClick={()=>{const label=window.prompt("Nome do breakpoint:","Laptop");if(!label)return;const width=window.prompt("Largura máxima em px:","1280");onAddBreakpoint(label,Number(width)||1280)}}>＋ Breakpoint personalizado</button><p className="ev-studio-help">Desktop é a base. Breakpoints personalizados, tablet e mobile herdam e sobrescrevem propriedades.</p></section><section><h4>Componentes mestres / Symbols</h4><div className="ev-master-list">{(blockComponents||[]).length?(blockComponents||[]).map(c=><div key={c.id}><div><b>{c.name}</b><small>{c.id}</small></div><button onClick={()=>onCreateInstance(c.id)}>＋ Instância</button></div>):<p className="ev-studio-help">Crie um componente mestre no Inspector de um bloco selecionado. Instâncias permanecem ligadas ao mestre no runtime.</p>}</div></section></>}
  {tab==="assets"&&<>
