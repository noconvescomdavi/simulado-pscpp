...
const EMPTY={
  ...
  config:{
    ...
    settings:{snapEnabled:true,snapPosition:.25,snapRotation:15,snapScale:.05,showGrid:true,showSectors:true,showBounds:false,showAxes:false,wireframe:false,xray:false,previewMode:"day",signalPresetMode:true},
    environment:{background:"#77b9e7",ambient:"#dcefff",ambientIntensity:.82,sun:"#fff0ce",sunIntensity:1.65,sunPosition:[12,8,-7],water:"#043f6b",exposure:1,fog:"#b5cbd6",fogEnabled:false,fogDensity:0},
...
export default function Admin3DEditor(){
  ...
  useEffect(()=>{
    const handler=e=>{
      ...
      if(e.key==="Escape"){setContextMenu(null);setCommandOpen(false)}
    };
  ...
  return <div className={styles.editorShell}>{commandOpen&&<div className={styles.diffPanel}><b>COMMAND PALETTE · Ctrl+K</b><input autoFocus placeholder="Buscar comando..." value={commandQuery} onChange={e=>setCommandQuery(e.target.value)}/>{commands.map(([name,fn])=><button key={name} onClick={()=>{setCommandOpen(false);fn()}}>{name}</button>)}<button onClick={()=>setCommandOpen(false)}>Fechar</button></div>}{contextMenu&&<div style={{position:"fixed",left:contextMenu.x,top:contextMenu.y,zIndex:99999,background:"#0b1824",border:"1px solid #2b5269",borderRadius:8,padding:6,display:"grid",gap:4,minWidth:180}}><button onClick={()=>{setSelected(contextMenu.id);setSelectedIds([contextMenu.id]);setContextMenu(null);setMode("translate")}}>Mover</button><button onClick={()=>{setSelected(contextMenu.id);setSelectedIds([contextMenu.id]);setContextMenu(null);setMode("rotate")}}>Rotacionar</button><button onClick={()=>{setSelected(contextMenu.id);setSelectedIds([contextMenu.id]);setContextMenu(null);duplicateObject()}}>Duplicar</button><button onClick={()=>{setSelected(contextMenu.id);setSelectedIds([contextMenu.id]);setContextMenu(null);setIsolateId(contextMenu.id)}}>Isolar</button><button onClick={()=>setContextMenu(null)}>Fechar</button></div>}
    ...
    {showDiff&&<div className={styles.diffPanel}><b>ALTERAÇÕES PENDENTES PARA O ALUNO</b>{publishDiff.length?publishDiff.slice(0,80).map((d,i)=><p key={i} data-kind={d.kind}>{d.label}</p>):<p>Nenhuma alteração detectada.</p>}{validation.errors.length>0&&<strong>{validation.errors.length} alerta(s) técnico(s) — publicação continua permitida.</strong>}</div>}<div className={styles.workspace}>
      ...
      <section className={styles.centerPane}>
        ...
        <div className={styles.viewportToolbar}>
          ...
          <button className={cfg.settings.previewMode==="night"?styles.active:""} onClick={()=>mutate(s=>{s.config.settings.previewMode=s.config.settings.previewMode==="night"?"day":"night"})}>{cfg.settings.previewMode==="night"?"☾ Noite · luzes":"☀ Dia · marcas"}</button>
          ...
          <button className={cfg.settings?.safePerformance?styles.active:""} onClick={()=>mutate(x=>{x.config.settings.safePerformance=!x.config.settings.safePerformance})}>⚡ Safe FPS</button>
        ...
        <div className={styles.cameraBar}><b>Saída:</b><button onClick={exportScreenshot4K}>Screenshot 4K</button><button onClick={exportVideo}>Gravar 10s</button><button onClick={exportScenePackage}>Exportar pacote</button><button onClick={cacheSceneOffline}>Preparar offline</button></div>
      ...
      <aside className={styles.inspector}>
        ...
        {tab==="object"&&obj&&<div className={styles.form}>
          ...
          <div className={styles.miniActions}>{obj.type==="model"&&<button onClick={autoRigAnchors}>Auto-rig anchors</button>}{obj.parentId&&obj.anchorName&&<button onClick={snapSelectedToAnchor}>Encaixar no anchor</button>}<button onClick={()=>patchObject({wireframe:!obj.wireframe})}>{obj.wireframe?"Sólido":"Wireframe"} por objeto</button></div>
          ...
          {obj.type==="model"&&<><div className={styles.vecTitle}>Pivô</div><div className={styles.vec}>{["X","Y","Z"].map((x,i)=><label key={x}>{x}<input type="number" step=".05" value={(obj.pivot||[0,0,0])[i]} onChange={e=>patchVec("pivot",i,e.target.value)}/></label>)}</div>
            ...
            <div className={styles.materialDiag}><b>Iluminação do objeto</b><span>Estes controles não alteram as texturas originais do GLB. São ajustes de renderização por objeto e também são publicados para o aluno.</span></div>
            <label>Brilho do objeto <input type="range" min=".05" max="2" step=".05" value={obj.renderAdjustments?.brightness??1} onChange={e=>patchObject({renderAdjustments:{...(obj.renderAdjustments||{}),brightness:Number(e.target.value)}})}/><small>{Math.round((obj.renderAdjustments?.brightness??1)*100)}%</small></label>
            <label>Exposição do objeto <input type="range" min="-4" max="2" step=".1" value={obj.renderAdjustments?.exposure??0} onChange={e=>patchObject({renderAdjustments:{...(obj.renderAdjustments||{}),exposure:Number(e.target.value)}})}/><small>{Number(obj.renderAdjustments?.exposure??0).toFixed(1)} EV</small></label>
            <label>Reflexo do ambiente <input type="range" min="0" max="2" step=".05" value={obj.renderAdjustments?.environment??1} onChange={e=>patchObject({renderAdjustments:{...(obj.renderAdjustments||{}),environment:Number(e.target.value)}})}/><small>{Math.round((obj.renderAdjustments?.environment??1)*100)}%</small></label>
            <div className={styles.miniActions}><button onClick={()=>patchObject({renderAdjustments:{brightness:.35,exposure:-2,environment:.18}})}>Noite em alto-mar</button><button onClick={()=>patchObject({renderAdjustments:{brightness:.75,exposure:-.5,environment:.6}})}>Crepúsculo</button><button onClick={()=>patchObject({renderAdjustments:{brightness:1,exposure:0,environment:1}})}>Restaurar iluminação</button></div>
        ...
        {tab==="scene"&&<div className={styles.form}>
          ...
          <h4>Ambiente</h4>
          <div className={styles.materialDiag}><b>Preset de sinalização da cena</b><span>Dia: exibe marcas diurnas e oculta luzes RIPEAM. Noite: exibe luzes RIPEAM e oculta marcas diurnas. O navio permanece visível.</span><label><input type="checkbox" checked={cfg.settings.signalPresetMode!==false} onChange={e=>mutate(s=>{s.config.settings.signalPresetMode=e.target.checked})}/> Aplicar automaticamente Dia/Noite</label></div>
          <div className={styles.miniActions}><button className={cfg.settings.previewMode==="day"?styles.active:""} onClick={()=>mutate(s=>{s.config.settings.previewMode="day"})}>☀ Preset Dia</button><button className={cfg.settings.previewMode==="twilight"?styles.active:""} onClick={()=>mutate(s=>{s.config.settings.previewMode="twilight"})}>◐ Crepúsculo</button><button className={cfg.settings.previewMode==="night"?styles.active:""} onClick={()=>mutate(s=>{s.config.settings.previewMode="night"})}>☾ Preset Noite</button></div>
          <div className={styles.miniActions}>{ENVIRONMENT_PRESETS.map(p=><button key={p.key} onClick={()=>applyEnvironmentPreset(p)}>{p.label}</button>)}</div>{[["background","Fundo"],["water","Água"],["ambient","Luz ambiente"],["sun","Sol"],["fog","Neblina"]].map(([k,l])=><label key={k}>{l}<input type="color" value={cfg.environment[k]} onChange={e=>mutate(s=>{s.config.environment[k]=e.target.value})}/></label>)}<label>Exposição<input type="range" min=".1" max="3" step=".05" value={cfg.environment.exposure} onChange={e=>mutate(s=>{s.config.environment.exposure=Number(e.target.value)})}/></label><label><span>FOG / Neblina</span><input type="checkbox" checked={cfg.environment.fogEnabled===true} onChange={e=>mutate(s=>{s.config.environment.fogEnabled=e.target.checked;if(e.target.checked&&!s.config.environment.fogDensity)s.config.environment.fogDensity=.012})}/></label>
          <label>Intensidade da neblina<input type="range" min="0" max=".08" step=".001" value={cfg.environment.fogDensity||0} onChange={e=>mutate(s=>{s.config.environment.fogDensity=Number(e.target.value);s.config.environment.fogEnabled=Number(e.target.value)>0})}/><small>{Math.round((cfg.environment.fogDensity||0)/.08*100)}%</small></label>
          <h4>Dependências da cena</h4><div className={styles.materialDiag}>{sceneDependencies.length?sceneDependencies.map(url=><span key={url}>{url} · hash {RIPEAM_ASSET_HASHES[String(url).split("?")[0]]||"externo"}</span>):<span>Sem assets externos.</span>}<span>Órfãos na biblioteca: {orphanAssets.length}</span><span>Grupos duplicados por hash: {duplicateAssets.length}</span></div>
