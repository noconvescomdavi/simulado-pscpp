      ...
      <aside className={styles.inspector}>
        ...
        {tab==="object"&&obj&&<div className={styles.form}>
          ...
          {obj.type==="model"&&<><div className={styles.vecTitle}>Pivô</div><div className={styles.vec}>{["X","Y","Z"].map((x,i)=><label key={x}>{x}<input type="number" step=".05" value={(obj.pivot||[0,0,0])[i]} onChange={e=>patchVec("pivot",i,e.target.value)}/></label>)}</div>
            ...
            <label>Exposição do objeto <input type="range" min="-3" max="2" step=".1" value={obj.renderAdjustments?.exposure??0} onChange={e=>patchObject({renderAdjustments:{...(obj.renderAdjustments||{}),exposure:Number(e.target.value)}})}/><small>{Number(obj.renderAdjustments?.exposure??0).toFixed(1)} EV</small></label>
            ...
            <div className={styles.miniActions}><button onClick={()=>patchObject({renderAdjustments:{brightness:.55,exposure:-1,environment:.35}})}>Preset noturno</button><button onClick={()=>patchObject({renderAdjustments:{brightness:.75,exposure:-.5,environment:.6}})}>Crepúsculo</button><button onClick={()=>patchObject({renderAdjustments:{brightness:1,exposure:0,environment:1}})}>Restaurar iluminação</button></div>
