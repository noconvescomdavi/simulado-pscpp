...
import {formatCurrencyFromCents,getResolvedTutorPaymentConfig} from "../../lib/payments";
...
export default async function TutorPage({searchParams}){
  ...
  const tutorConfig=await getResolvedTutorPaymentConfig();
  const tutorPrice=formatCurrencyFromCents(tutorConfig.priceCents);
  ...
  return <><StudentHeader active="tutor"/><main className={styles.page}>
    <section className={styles.hero}><div><span>CONTRAMESTRE — TUTOR ACADÊMICO ESPECIALIZADO</span><h1>⚓ CONTRAMESTRE</h1><p>Um tutor focado exclusivamente no universo PSCPP e marítimo. Tire dúvidas, peça explicações e revise conceitos.</p></div><div className={styles.badge}>PACOTE ADICIONAL<br/><strong>{tutorPrice}/mês</strong></div></section>
    {access?.active?<TutorChat conversations={conversations} initialConversation={selected} remaining={Math.max(0,AI_TUTOR_DAILY_LIMIT-Number(usage.questions||0))}/>:<section className={styles.locked}>
      <div className={styles.lockIcon}>✨</div><h2>Adicione o CONTRAMESTRE à sua preparação</h2>
      <p>Este recurso não está incluído no plano principal da ESTIBORDO. O acesso ao CONTRAMESTRE é vendido separadamente por <strong>{tutorPrice} por mês</strong>.</p>
      ...
      <form action="/api/payments/mercado-pago/tutor-checkout" method="post"><button>Comprar CONTRAMESTRE — {tutorPrice}/mês</button></form>
