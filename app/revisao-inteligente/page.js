...
export default async function Page(){
 ...
 <div className={styles.list}>{items.length?items.map((x)=><article key={x.subject+"|"+x.topic_code+"|"+x.topic}><div><small>{x.subject_label} · {x.stage}</small><h2>{x.topic}</h2><p>{x.answers} respostas · {x.errors} erros · {x.accuracy}% de acerto</p></div><div className={styles.priority}><b>{x.priority}</b><span>{x.is_due?"revisar agora":"agendado"}</span><small>{x.due_at?new Date(x.due_at).toLocaleDateString("pt-BR"):""}</small></div><div><a href={x.href}>Resolver {x.suggested_questions} questões →</a><ReviewActions sourceKey={x.source_key} dueAt={x.due_at}/></div></article>):<div className={styles.empty}>Ainda não há erros suficientes para montar sua fila adaptativa.</div>}</div>
 </main></>;
}
