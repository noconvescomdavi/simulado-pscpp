...
export default function PracticalSimulator(){
 ...
 function execute(){setPhase("execute");setRunning(true);setTrack([{lat:state.lat,lon:state.lon}]);setVoyageLog([])}
 function reset(){setPhase("setup");setRoute([]);setTrack([]);setFailed(false);setFailure(null);setAlarms([]);setRunning(false);setAnchor({down:false,cableM:0});setCommands([]);setVoyageLog([]);t.current=0}
 useEffect(()=>{if(view.follow&&phase==="execute")setView(v=>({...v,lat:state.lat,lon:state.lon}))},[state.lat,state.lon,view.follow,phase]);
 useEffect(()=>{if(!running||failed)return;const id=setInterval(()=>{t.current++;setState(s=>{const n=stepVessel(s,ship,1,tugForce(tugs)),m=monitorRoute(n,route),depth=trainingDepthAt(n.lat,n.lon);setTrack(q=>[...q.slice(-3599),{lat:n.lat,lon:n.lon}]);const updated={...n,xte:m.xte,depth};const sf=navigationSafety({state:updated,ship,xte:m.xte});setVoyageLog(v=>[...v.slice(-7199),snapshot(updated,{ukc:sf.ukc,squat:sf.squat})]);return updated})},1000);setTraffic(v=>v.map(q=>stepVessel({...q,stw:q.sog,hdg:q.cog,current:0,currentDir:0,rudder:0,engine:"Half Ahead"},ship,1)))},1000);return()=>clearInterval(id)},[running,failed,ship,route,tugs]);
 ...
 return <main className="practical">
  ...
  <div className="workspace">
   ...
   <section className="bottomPanel"><div><b>PILOT CARD</b><span>{ship.name} · {ship.type}</span><span>LOA {ship.loa} m · Beam {ship.beam} m</span><span>Draft {ship.draft} m · Air draft {ship.air} m</span></div><div><b>ASSIGNMENT</b><span>{scenario}</span><span>Charts: 1511 · 1512 · 1515</span><span>Minimum planned UKC: {ship.minU.toFixed(2)} m</span></div><div><b>PHASE</b><strong>{phase.toUpperCase()}</strong>{phase==="plan"&&<button onClick={begin}>Lock route & briefing →</button>}{phase==="brief"&&<button onClick={execute}>Briefing complete · Execute →</button>}{phase==="debrief"&&<><span>Max XTE: {debrief(voyageLog,failure).maxXte.toFixed(0)} m</span><span>Min UKC: {debrief(voyageLog,failure).minUkc?.toFixed(2)??"-"} m</span><span>{failure?`DISQUALIFIED · ${failure.label}`:"MANOEUVRE COMPLETED"}</span><button onClick={reset}>New assessment</button></>}</div></section>
  </div>}
 </main>
}
