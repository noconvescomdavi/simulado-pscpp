...
export default function PracticalSimulator(){
 ...
 const environment=useMemo(()=>seededScenario(151115121515),[]);\n const [state,setState]=useState({lat:-22.973,long:-43.139,hdg:340,cog:340,sog:7.0,stw:6.7,depth:19.2,tide:.72,...environment,rudder:0,engine:"Half Ahead",xte:0,rot:0});
 const t=useRef(0);
 ...
 function addWP(e){if(phase!=="plan")return;const r=e.currentTarget.getBoundingClientRect();const x=(e.clientX-r.left)/r.width,y=(e.clientY-r.top)/r.height;setRoute(v=>[...v,screenToGeo(x,y)])}
 ...
 useEffect(()=>{if(!running||failed)return;const id=setInterval(()=>{t.current++;setState(s=>{const n=stepVessel(s,ship,1),m=monitorRoute(n,route),depth=trainingDepthAt(n.lat,n.long);return {...n,xte:m.xte,depth}})},1000);return()=>clearInterval(id)},[running,failed,ship,route]);
 useEffect(()=>{if(phase!=="execute")return;setAlarms(safety.alarms.map(a=>({level:a.severity.toUpperCase(),text:a.message})));if(safety.disqualified){setFailed(true);setRunning(false)}},[phase,safety.disqualified,safety.criticalCode,ukc,state.xte]);
