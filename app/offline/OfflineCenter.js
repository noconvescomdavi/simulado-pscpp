...
import {
  ...
  downloadOfflinePack, finishOfflineExam, getOfflineNotebook, getOfflineQuestion,
  getOfflineStatus, hydrateOfflineExam, getOfflineExam, syncOfflineQueue
...
export default function OfflineCenter(){
  ...
  async function refresh(){try{setStatus(await getOfflineStatus())}catch{}}
  useEffect(()=>{
    refresh();
  ...
  async function install(){
    ...
    try{const next=await downloadOfflinePack();setStatus(next);setMessage(`✓ ${next.questions.toLocaleString("pt-BR")} questões disponíveis offline.`)}
    catch(e){setMessage(e.message||"Falha ao preparar conteúdo offline.")}
    finally{setBusy("")}
  }
  async function sync(){
    setBusy("sync");setMessage("");
    try{const r=await syncOfflineQueue();await refresh();setMessage(r.offline?"Sem conexão. Suas alterações continuam protegidas no aparelho.":`✓ ${r.synced||0} alteração(ões) sincronizada(s).`)}
  ...
  async function makeNotebook(){
    ...
    try{const n=await createOfflineNotebook({subjects:[subject],count});history.replaceState(null,"",`/offline?mode=notebook&id=${n.id}`);await openExisting("notebook",n.id)}
    catch(e){setMessage(e.message)}
    finally{setBusy("")}
  }
  async function makeExam(){
    setBusy("exam");setMessage("");
    try{const e=await createOfflineExam({subject,count:Math.min(100,Math.max(10,count)),title:`Simulado offline · ${SUBJECTS.find(x=>x[0]===subject)?.[1]||subject}`});history.replaceState(null,"",`/offline?mode=exam&id=${e.id}`);await openExisting("exam",e.id)}
    catch(e){setMessage(e.message)}
    finally{setBusy("")}
  }
  async function choose(letter){
    ...
    try{
      ...
      setAnswer(saved);setItem(current=>({...current,answers:{...(current.answers||{}),[mode==="notebook"?`${q.subject}:${q.id}`:String(q.id)]:saved}}));await refresh();
    }catch(e){setMessage(e.message)}
    finally{setBusy("")}
  }
  function next(){
  ...
  async function finish(){
    ...
    try{await finishOfflineExam(item.id,"manual");const result=calc(item.questions.length,item.answers);setItem(x=>({...x,status:"completed",result}));setMessage("Simulado finalizado no dispositivo. O resultado será enviado ao servidor quando houver conexão.");await refresh()}
    finally{setBusy("")}
  }
  ...
  if(mode&&item){
    ...
    if(result)return <main style={{maxWidth:900,margin:"0 auto",padding:"28px 18px"}}>
      ...
      <p><strong>Salvo no aparelho.</strong> {navigator.onLine?"A sincronização automática será executada.":"Será sincronizado quando a internet voltar."}</p>
    </main>;

    const q=item.questions[index];const saved=answer||(mode==="notebook"?item.answers?.[`${q?.subject}:${q?.id}`]:item.answers?.[String(q?.id)]);
  ...
  return <main style={{maxWidth:1050,margin:"0 auto",padding:"30px 18px 80px"}}>
    <p style={{fontWeight:900,letterSpacing:1}}>ESTIBORDO OFFLINE</p>
    <h1>Central Offline</h1>
    <p>Prepare o dispositivo antes do embarque. Questões, simulados, cadernos e progresso continuam funcionando sem conexão e são sincronizados depois.</p>
    ...
    <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,margin:"24px 0"}}>
      <article><small>Status</small><h3>{navigator.onLine?"Online":"Offline"}</h3></article>
      ...
      <article><small>Última preparação</small><h3>{status?.downloaded_at?new Date(status.downloaded_at).toLocaleDateString("pt-BR"):"—"}</h3></article>
    </section>
    <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:30}}>
      <button onClick={install} disabled={busy==="download"}>{busy==="download"?"Baixando…":status?.installed?"Atualizar conteúdo offline":"Preparar para ficar offline"}</button>
      <button onClick={sync} disabled={busy==="sync"||!navigator.onLine}>{busy==="sync"?"Sincronizando…":"Sincronizar agora"}</button>
    </div>
    <section style={{padding:"22px",border:"1px solid #294351",borderRadius:18}}>
      ...
      {!status?.installed&&<p>Primeiro use <strong>Preparar para ficar offline</strong>.</p>}
    </section>
  </main>;
}
