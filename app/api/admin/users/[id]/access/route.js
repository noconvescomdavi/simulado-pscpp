...
export async function POST(req, context) {
  ...
  await query(`
    INSERT INTO user_access(user_id, product_code, status, lifetime, activated_at, revoked_at)
    VALUES(
      $1,
      'pscpp-vitalicio',
      $2,
    ...
    DO UPDATE SET
      ...
      lifetime = TRUE,
      activated_at = CASE
        WHEN EXCLUDED.status = 'active' THEN COALESCE(user_access.activated_at, NOW())
  ...
  await query(
    ...
    [admin.id, `admin_access_${action}:${id}`]
  );

  const label = action === "active" ? "Licença ativada." : action === "revoked" ? "Licença revogada." : "Licença marcada como pendente.";
