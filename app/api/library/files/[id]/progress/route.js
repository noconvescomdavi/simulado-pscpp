...
export async function POST(request,{params}){
  ...
  const result=await query("update student_library_files set last_page=$3,progress_percent=greatest(progress_percent,$4),last_opened_at=now(),updated_at=now() where id=$1 and user_id=$2 returning id,last_page,progress_percent,last_opened_at",[id,session.id,page,progress]);
