// Migrador canônico V2: preserva IDs e conteúdo das questões existentes.
...
const confidenceCounts=new Map(), unmappedReasons=new Map();
...
for(const q of questions){
  ...
  q.taxonomy={subject_id:subjectId,subject_slug:slug,bibliography_id:hit.bibliography_id,chapter_id:chapterId,topic_id:topicId,subtopic_id:null};
  q.tracking={
    subject_slug:slug,
  ...
  migrated++;inc(confidenceCounts,hit.confidence||"unknown");
}
...
const countsByChapter=new Map();let v2=0,legacy=0;
for(const q of questions){if(q?.taxonomy?.subject_slug===slug){v2++;inc(countsByChapter,q.taxonomy.chapter_id);}else legacy++;}
const coverage=[];
for(const work of catalog){
  const available=sourceAvailability[work.key]!==false;
  for(const section of work.sections||[]){
    const chapterId=`${work.key}::${section.key}`,count=countsByChapter.get(chapterId)||0;
    let status;if(!available)status="fonte_pendente";else if(count===0)status="critico";else if(count<25)status="insuficiente";else if(count<=50)status="meta_atingida";else status="cobertura_ampla";
...
const summary={total:questions.length,v2,legacy,migrated,already_v2:alreadyV2,source_locator_improved:sourceLocatorImproved,mapped_confidence:Object.fromEntries(confidenceCounts),unmapped_samples:unmappedSamples.length,coverage:{critical:coverage.filter(x=>x.status==="critico").length,insufficient:coverage.filter(x=>x.status==="insuficiente").length,goal:coverage.filter(x=>x.status==="meta_atingida").length,wide:coverage.filter(x=>x.status==="cobertura_ampla").length,source_pending:coverage.filter(x=>x.status==="fonte_pendente").length}};
