...
export function makeQuestionsFromPairs({bank,subjectId,subjectSlug,prefix,bibliographyId,sectionKey,pairs,source,need,module,topicCodePrefix='EXP'}){
  ...
  for(let i=0;i<need;i++){
    ...
    else {question=`Assinale a associação tecnicamente correta conforme ${section.chapter||section.label}.`;correct=`${term} — ${definition}`;choices=pairs.map((p,j)=>`${p[0]} — ${pairs[(j+1)%pairs.length][1]}`);choices[idx]=correct;style='Análise/contraste';explanation=`A associação correta é “${term} — ${definition}”; as demais opções trocam conceitos e descrições.`;}
