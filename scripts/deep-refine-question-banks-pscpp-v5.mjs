...
const TARGET_PER_SUBJECT=380;
const ADD_PER_SUBJECT=0;
const PATTERNS=[
...
function meta(base,style,difficulty,tag,derived){
 return {
   ...
   tags:[...new Set([...(base.tags||[]),'pscpp-style-v5',tag].filter(Boolean))],
   provenance:{method:'pscpp-deep-refine-v5',derived_from_question_ids:derived||[base.id],benchmark:'PSCPP/DPC 2006, 2008, 2011, 2012 e 2023'}
 };
}
...
function makeAssertions(id,bases,seed,want){
 ...
 return {id,...meta(bases[0],'Assertivas I–IV','Difícil','formato-pscpp-assertivas-v5',bases.map(x=>x.id)),
  topic_code:`PSCPP.V5.AST.${String(seed+1).padStart(5,'0')}`,
...
function makeVF(id,bases,seed,want){
 ...
 return {id,...meta(bases[0],'Sequência V/F','Difícil','formato-pscpp-vf-v5',bases.map(x=>x.id)),
  topic_code:`PSCPP.V5.VF.${String(seed+1).padStart(5,'0')}`,
...
function makeIncorrect(id,bases,seed,want){
 ...
 return {id,...meta(bases[0],'Assinale a incorreta','Difícil','formato-pscpp-incorreta-v5',bases.map(x=>x.id)),
  topic_code:`PSCPP.V5.INC.${String(seed+1).padStart(5,'0')}`,
...
function uniqueStems(qs){
 ...
 for(const q of qs){let n=norm(q.question);if(!seen.has(n)){seen.set(n,q.id);continue;}if(!(q.tags||[]).includes('pscpp-style-v5'))continue;
...
for(const sub of SUBJECTS){
 ...
 for(let i=0;i<ids.length;i++){
   ...
   q.provenance.replaces_question_id=old.id;q.provenance.rewrite_reason='residual-medium-quality-v5';qs[ix]=q;rewritten++;
 }
 ...
 for(let i=0;i<ADD_PER_SUBJECT;i++){
   ...
   q.provenance.method='pscpp-expansion-v5';added.push(q);
 }
 qs.push(...added);
 ...
 for(let ix=0;ix<qs.length;ix++){
   const q=qs[ix];
   if(!(q.tags||[]).includes('pscpp-style-v5')||rawUnique(q))continue;
   ...
   if(bases.length===4){
     ...
     repaired.provenance.repair_reason='duplicate-options-after-v5-generation';
     qs[ix]=repaired;
   }
 }
...
fs.writeFileSync(path.join(rdir,'pscpp-deep-refine-v5.json'),JSON.stringify(report,null,2)+'\n');
