...
function sessionResult(session) {
  ...
  return {
    ...
    score_percent: answered ? Math.round((correct / answered) * 10000) / 100 : 0,
...
function activePayload(session) {
  ...
  const questions = ids.map((id) => getQuestion(session.subject, id)).filter(Boolean).map(publicQuestion);
  return {
    id: session.id,
...
async function lastSession(userId, subject, executor = query) {
...
async function availability(userId, subject, executor = query) {
  ...
  if (intervalDays === 0) return { can_start: true, next_available_at: null, interval_days: 0 };
  const result = await executor(
...
export async function finalizeExamSession(client, session, requestedReason = "manual") {
  ...
  const expired = typeof session.is_expired === "boolean"
    ? session.is_expired
  ...
  const durationSeconds = Math.max(
    0,
    Math.min(EXAM_DURATION_SECONDS, Math.round((Date.now() - new Date(session.started_at).getTime()) / 1000))
  );
  ...
  if (!attemptId) {
    const attempt = await client.query(
      ...
      [
        ...
        session.subject,
        answered ? Math.round((correct / answered) * 10000) / 100 : 0,
        correct,
        answered - correct,
        answered,
        durationSeconds,
  ...
  const updated = await client.query(
    `update exam_sessions
        set status=$2,finished_at=now(),finish_reason=$3,attempt_id=$4,
            answered_count=$5,correct_count=$6,current_index=$5,updated_at=now()
      where id=$1
      returning *`,
...
export async function getExamState(userId, rawSubject) {
  ...
  return withTransaction(async (client) => {
    const current = await client.query(
      `select *, expires_at <= now() as is_expired from exam_sessions
    ...
    return { state: "available", ...(await availability(userId, subject, client.query.bind(client))) };
  });
}
...
export async function startExam(userId, rawSubject) {
  ...
  if (!bank?.questions?.length) {
    return { error: "Matéria sem banco de questões.", code: "QUESTION_BANK_NOT_FOUND", status: 404 };
  }

  return withTransaction(async (client) => {
    await client.query("select pg_advisory_xact_lock(hashtext($1))", [`exam:${userId}:${subject}`]);
    const current = await client.query(
      `select *, expires_at <= now() as is_expired from exam_sessions
        where user_id=$1 and subject=$2 and status='in_progress'
        order by started_at desc
    ...
    if (existing) await finalizeExamSession(client, existing, "timeout");
    ...
    const ids = shuffle(bank.questions.map((question) => question.id)).slice(0, Math.min(EXAM_SIZE, bank.questions.length));
...
export async function submitExamAnswer({ userId, rawSubject, sessionId, questionId, selectedAnswer, responseTimeMs }) {
  const subject = normalizeSubject(rawSubject);
  return withTransaction(async (client) => {
    ...
    if (session.status !== "in_progress") {
      return { error: "Este simulado já foi finalizado.", code: "EXAM_FINISHED", status: 409, result: sessionResult(session) };
    }
    if (session.is_expired) {
      const result = await finalizeExamSession(client, session, "timeout");
      const available = await availability(userId, subject, client.query.bind(client));
      return { error: "O tempo do simulado terminou.", code: "EXAM_EXPIRED", status: 409, result: { ...result, ...available } };
    }

    const question = getQuestion(subject, questionId);
    const ids = questionIds(session.question_ids);
    if (!question || !ids.includes(String(questionId))) {
      return { error: "Questão inválida para este simulado.", code: "INVALID_QUESTION", status: 400 };
    }

    const duplicate = await client.query(
      `select selected_answer,is_correct from exam_session_answers
    ...
    if (duplicate.rowCount) {
      ...
      return {
        ...
        answered_count: Number(session.answered_count),
    ...
    if (String(question.id) !== String(expectedQuestionId)) {
      return { error: "Salve as questões na ordem apresentada.", code: "OUT_OF_SEQUENCE", status: 409 };
    }
    ...
    await client.query(
      ...
      [session.id, userId, question.id, position, selectedAnswer, isCorrect, responseTimeMs]
    );
    await client.query(
      `insert into question_answers
       (user_id,session_id,question_id,subject,selected_answer,is_correct,response_time_ms)
       values($1,$2,$3,$4,$5,$6,$7)`,
      [userId, session.id, question.id, subject, selectedAnswer, isCorrect, responseTimeMs]
    );
    await client.query(
      ...
      [userId, question.id, subject, isCorrect ? 1 : 0, isCorrect ? 0 : 1]
    );
    await client.query(
    ...
    const updated = await client.query(
      `update exam_sessions
          set answered_count=answered_count+1,
              ...
              updated_at=now()
        where id=$1
        returning answered_count,correct_count`,
    ...
    return {
      ...
      explanation: question.explanation,
      source: question.source,
      answered_count: Number(updated.rows[0].answered_count),
      correct_count: Number(updated.rows[0].correct_count),
    };
  });
}
...
export async function finalizeExpiredExams() {
  const expired = await query(
    `select id,user_id,subject from exam_sessions
