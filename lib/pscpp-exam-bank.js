...
function enrich(question, subject) {
  ...
  return {
    ...
    pscpp_origin: question.pscpp_origin || "generated_pscpp",
...
export function getPscppPool() {
  return Object.keys(SUBJECT_QUOTAS).flatMap(subject => {
...
export function getPscppQuestion(id) {
  ...
  const q=(getQuestionBank(subject)?.questions||[]).find(x=>String(x.id)===sourceId);
...
export function buildPscppExam(seed=Date.now()) {
  ...
  for(const [subject,quota] of Object.entries(SUBJECT_QUOTAS)){
    const candidates=ranked((getQuestionBank(subject)?.questions||[]).map(q=>enrich(q,subject)),seed+":"+subject);
