...
export async function ensureRipeam3DSchema(){
  ...
  await query(
    "create table if not exists ripeam_3d_scene_publications("+
    "scene_key text primary key, scene_id uuid, snapshot jsonb not null, published_at timestamptz not null default now())"
  );
  await query(
    "insert into ripeam_3d_scene_publications(scene_key,scene_id,snapshot,published_at) "+
    "select scene_key,id,jsonb_build_object('id',id,'scene_key',scene_key,'title',title,'rule_ref',rule_ref,'card_title',card_title,'description',description,'status','published','config',config),updated_at "+
    "from ripeam_3d_scenes where status='published' on conflict(scene_key) do nothing"
  );
  await query(
...
export function sanitizeScene(input={}){
  const objects=(Array.isArray(input.objects)?input.objects:[]).slice(0,400).map((o,i)=>({
    ...
    type:["model","pointLight","spotLight","directionalLight","shape","cable","measure","hotspot","flag","group"].includes(o.type)?o.type:"model",
    ...
    flagCode:txt(o.flagCode,8),
    anchorName:txt(o.anchorName,80),
    flagColors:(Array.isArray(o.flagColors)?o.flagColors:[]).slice(0,4).map(x=>color(x,"#ffffff")),
    ...
    material:{
      mode:o.material?.mode==="custom"?"custom":"original",
      ...
      doubleSide:bool(o.material?.doubleSide,false)
    },
    // Per-object render controls are part of the authored scene. Persist them in
    // draft/version/publication snapshots so "Atualizar aluno" is lossless.
    renderAdjustments:{
      brightness:Math.max(.02,Math.min(3,n(o.renderAdjustments?.brightness,1))),
      exposure:Math.max(-4,Math.min(3,n(o.renderAdjustments?.exposure,0))),
      environment:Math.max(0,Math.min(3,n(o.renderAdjustments?.environment,1)))
    },
    lod:{
  ...
  return {
    ...
    settings:{
      ...
      showBounds:bool(settings.showBounds,false),
      showAxes:bool(settings.showAxes,false),
      wireframe:bool(settings.wireframe,false),
      xray:bool(settings.xray,false),
      previewMode:["day","twilight","night"].includes(settings.previewMode)?settings.previewMode:"day",
      signalPresetMode:settings.signalPresetMode!==false
    },
    environment:{
      ...
      fog:color(e.fog,"#07121d"),fogEnabled:bool(e.fogEnabled,false),fogDensity:Math.max(0,Math.min(.2,n(e.fogDensity,0)))
    },
...
export async function listRipeam3DScenes({publishedOnly=false}={}){
  ...
  if(publishedOnly){
    return (await query(
      "select snapshot->>'id' as id,scene_key,snapshot->>'title' as title,snapshot->>'rule_ref' as rule_ref,"+
      "snapshot->>'card_title' as card_title,snapshot->>'description' as description,'published' as status,"+
      "snapshot->'config' as config,published_at as created_at,published_at as updated_at "+
      "from ripeam_3d_scene_publications order by published_at desc"
    )).rows;
  }
  return (await query("select id,scene_key,title,rule_ref,card_title,description,status,config,created_at,updated_at from ripeam_3d_scenes order by updated_at desc")).rows;
}

export async function listRipeam3DPublications(){
  await ensureRipeam3DSchema();
  return (await query("select scene_key,scene_id,snapshot,published_at from ripeam_3d_scene_publications order by published_at desc")).rows;
}
...
export async function publishRipeam3DScene(input={}){
  await ensureRipeam3DSchema();
  const row=await saveRipeam3DScene({...input,status:"review",skipVersion:false,versionLabel:input.versionLabel||("Publicação "+new Date().toISOString())});
  const snapshot={id:row.id,scene_key:row.scene_key,title:row.title,rule_ref:row.rule_ref,card_title:row.card_title,description:row.description,status:"published",config:row.config};
  await query(
    "insert into ripeam_3d_scene_publications(scene_key,scene_id,snapshot,published_at) values($1,$2,$3::jsonb,now()) "+
    "on conflict(scene_key) do update set scene_id=excluded.scene_id,snapshot=excluded.snapshot,published_at=now()",
    [row.scene_key,row.id,JSON.stringify(snapshot)]
  );
  return {...row,status:"review",publishedSnapshot:snapshot};
}

export async function deleteRipeam3DScene(id){
  await ensureRipeam3DSchema();
  const row=(await query("select scene_key from ripeam_3d_scenes where id=$1",[id])).rows[0];
  await query("delete from ripeam_3d_scenes where id=$1",[id]);
  if(row?.scene_key)await query("delete from ripeam_3d_scene_publications where scene_key=$1",[row.scene_key]);
}
