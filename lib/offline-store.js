...
import { buildHistoricalExam, buildDiversePscppExam } from "./historical-exam-blueprint";
...
function calcResult(total,answers={}){const rows=Object.values(answers||{}),answered=rows.length,correct=rows.filter(x=>x?.is_correct).length,errors=Math.max(0,answered-correct),blank=Math.max(0,Number(total||0)-answered),score_percent=total?Math.round((correct/Number(total))*10000)/100:0;return{answered,correct,errors,blank,total_questions:Number(total||0),score_percent,grade_10:Math.round((score_percent/10)*100)/100,completed:answered>=Number(total||0)}}
...
function pscppOfflineSelection(pool,count){
  if(Number(count)!==100)throw new Error("O Simulado PSCPP deve conter exatamente 100 questões.");
  return buildDiversePscppExam(pool,{seed:uuid()});
}
