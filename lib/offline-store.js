...
const DB_VERSION=2;
const MANIFEST_CHECK_TTL_MS=6*60*60*1000; // at most four version checks/day/device
let manifestCheckInFlight=null;
...
export async function ensureOfflinePackCurrent({force=false}={}){
  ...
  if(manifestCheckInFlight&&!force)return manifestCheckInFlight;
  const run=async()=>{
    try{await navigator.storage?.persist?.()}catch{}
    const local=await getOfflineStatus();
    const lastCheck=await get(STORES.meta,"manifest_check");
    const checkedAt=Number(lastCheck?.at||0);
    if(!force&&local.installed&&checkedAt&&Date.now()-checkedAt<MANIFEST_CHECK_TTL_MS){
      return {status:local,updated:false,skipped:true};
    }
    const manifestResponse=await fetch("/api/offline/manifest",{cache:"no-store"});
    const manifest=await manifestResponse.json().catch(()=>({}));
    if(!manifestResponse.ok)throw new Error(manifest.error||"Não foi possível verificar o banco offline.");
    await put(STORES.meta,{key:"manifest_check",at:Date.now(),version:manifest.version||null});
    const needsUpdate=
      force||
      !local.installed||
  ...
  manifestCheckInFlight=run().finally(()=>{manifestCheckInFlight=null});
  return manifestCheckInFlight;
}
