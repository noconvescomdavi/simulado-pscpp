...
export async function installOfflinePack(pack){
  ...
  const previous=await all(STORES.questions);
  ...
  for(const row of previous){
    if(incomingSubjects.includes(normalize(row.subject)))questions.delete(row.key);
  }
...
async function replaceOfflineSubject(subject,title,questions,version){
  const [previous,oldPack]=await Promise.all([all(STORES.questions),get(STORES.meta,"pack")]);
  const db=await openOfflineDb();
  const tx=db.transaction([STORES.questions,STORES.meta],"readwrite");
  const store=tx.objectStore(STORES.questions);
  for(const row of previous)if(normalize(row.subject)===normalize(subject))store.delete(row.key);
