...
function uuid(){return globalThis.crypto?.randomUUID?.()||`offline-${Date.now()}-${Math.random().toString(16).slice(2)}`}
...
export async function getLatestOfflineExam(subject){
  const items=(await all(STORES.exams)).filter(x=>x.subject===subject).sort((a,b)=>String(b.started_at).localeCompare(String(a.started_at)));return items[0]||null;
}
