...
function shuffle(items){const out=[...items];for(let i=out.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[out[i],out[j]]=[out[j],out[i]]}return out}\nfunction pscppOfflineSelection(pool,count){
  const valid=pool.filter(q=>q&&q.active!==false&&!q.annulled&&q.validation_status!=="rejected");
  const generated=shuffle(valid.filter(q=>q.pscpp_origin==="generated_pscpp"));
  const official=shuffle(valid.filter(q=>q.pscpp_origin==="official_exam"));
  const generatedTarget=generated.length>=65?Math.min(count,65):Math.min(count,generated.length);
  const officialTarget=Math.min(count-generatedTarget,official.length);
  const picked=[...generated.slice(0,generatedTarget),...official.slice(0,officialTarget)];
  const used=new Set(picked.map(q=>String(q.id)));
  if(picked.length<count){
    for(const q of shuffle(valid)){
      if(used.has(String(q.id)))continue;
      picked.push(q);used.add(String(q.id));
...
export async function createOfflineExam({subject,count=100,filters={},title=null,planTask=null}){
  ...
  const wanted=Math.min(100,Math.max(1,Number(count)||100));\n  const selected=normalize(subject)==="simulado-pscpp"?pscppOfflineSelection(pool,wanted):shuffle(pool).slice(0,wanted);
