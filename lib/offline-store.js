...
function shuffle(items){const out=[...items];for(let i=out.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[out[i],out[j]]=[out[j],out[i]]}return out}\nfunction pscppOfflineSelection(pool,count){\n  const quotas={manobrabilidade:14,"arte-naval":12,"navegacao-aguas-restritas":16,"situacoes-de-manobra-ripeam":14,"legislacao-regulamentacao":14,"meteorologia-oceanografia":12,comunicacoes:8,"conhecimentos-gerais":10};\n  const picked=[];const used=new Set();\n  for(const [source,quota] of Object.entries(quotas)){for(const q of shuffle(pool.filter(x=>x.source_subject===source)).slice(0,quota)){picked.push(q);used.add(q.id)}}\n  if(picked.length<count)for(const q of shuffle(pool)){if(!used.has(q.id)){picked.push(q);used.add(q.id)}if(picked.length>=count)break}\n  return shuffle(picked).slice(0,count);\n}
