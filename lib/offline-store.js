...
export async function installOfflinePack(pack){
  ...
  tx.objectStore(STORES.meta).put({
    key:"pack",
    version:pack?.version||oldPack?.version||"unknown",
    downloaded_at:nowIso(),
    subjects:mergedSubjects,
    complete:Boolean(pack?.complete||oldPack?.complete)
  });
...
export async function downloadOfflinePack(subjects=[]){
  const wanted=subjects.length?[...new Set(subjects.map(normalize).filter(Boolean))]:null;
  ...
  const banks=(manifest.banks||[]).filter(bank=>!wanted||wanted.includes(normalize(bank.subject)));
  if(!banks.length)throw new Error("Nenhum banco de questões disponível para armazenamento offline.");
  let status=null;
  for(const bank of banks){
    const response=await fetch("/api/offline/bootstrap?subjects="+encodeURIComponent(bank.subject),{cache:"no-store"});
    const payload=await response.json().catch(()=>({}));
    if(!response.ok)throw new Error(payload.error||"Não foi possível baixar o banco "+bank.subject+".");
    status=await installOfflinePack(payload);
  }
  status=status||await getOfflineStatus();
  await put(STORES.meta,{
    key:"pack",
    version:manifest.version||status.version||"unknown",
    downloaded_at:nowIso(),
    subjects:banks.map(bank=>bank.subject),
    complete:!wanted&&banks.length===(manifest.banks||[]).length
  });
...
export async function getOfflineStatus(){
  ...
  return {installed:Boolean(pack),complete:Boolean(pack?.complete),version:pack?.version||null,downloaded_at:pack?.downloaded_at||null,subjects:pack?.subjects||[],questions:questions.length,pending:queue.length,notebooks:notebooks.length,exams:exams.length};
}
