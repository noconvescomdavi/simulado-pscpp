...
import { buildHistoricalExam, buildPscppFixedExam } from "./historical-exam-blueprint";
import {standaloneEnabled} from "./standalone/runtime";
import {appendStandaloneChange} from "./standalone/journal";
import {bundledContentPack} from "./standalone/content-pack";
import {hasAndroidLocalStore,androidGet,androidPut,androidAll,androidDelete} from "./standalone/android-store";
...
function syncEventPriority(type){
  if(type==="notebook.create")return 0;
  if(type==="notebook.answer")return 1;
  if(type==="exam.snapshot")return 1;
  if(type==="study.task"||type==="reading.progress"||type==="library.progress")return 2;
  return 9;
}
...
export function openOfflineDb(){
  ...
  return new Promise((resolve,reject)=>{
    ...
    request.onupgradeneeded=()=>{
      ...
      if(!db.objectStoreNames.contains(STORES.questions)){const s=db.createObjectStore(STORES.questions,{keyPath:"key"});s.createIndex("subject","subject",{unique:false})}else{const s=request.transaction.objectStore(STORES.questions);if(!s.indexNames.contains("subject"))s.createIndex("subject","subject",{unique:false})}
...
async function put(store,value){if(hasAndroidLocalStore()&&store===STORES.exams)return androidPut("exam_sessions",value.id,value);if(hasAndroidLocalStore()&&store===STORES.notebooks)return androidPut("notebooks",value.id,value);const db=await openOfflineDb();const tx=db.transaction(store,"readwrite");tx.objectStore(store).put(value);await txDone(tx);db.close();emit();return value}
async function get(store,key){if(hasAndroidLocalStore()&&store===STORES.exams)return androidGet("exam_sessions",key);if(hasAndroidLocalStore()&&store===STORES.notebooks)return androidGet("notebooks",key);const db=await openOfflineDb();const tx=db.transaction(store,"readonly");const value=await req(tx.objectStore(store).get(key));await txDone(tx);db.close();return value||null}
async function all(store){if(hasAndroidLocalStore()&&store===STORES.exams)return androidAll("exam_sessions");if(hasAndroidLocalStore()&&store===STORES.notebooks)return androidAll("notebooks");const db=await openOfflineDb();const tx=db.transaction(store,"readonly");const value=await req(tx.objectStore(store).getAll());await txDone(tx);db.close();return value||[]}
async function count(store){const db=await openOfflineDb();const tx=db.transaction(store,"readonly");const value=await req(tx.objectStore(store).count());await txDone(tx);db.close();return Number(value)||0}

async function allByIndex(store,indexName,value){const db=await openOfflineDb();const tx=db.transaction(store,"readonly");const idx=tx.objectStore(store).index(indexName);const rows=await req(idx.getAll(value));await txDone(tx);db.close();return rows||[]}
async function deleteByIndex(store,indexName,value){const db=await openOfflineDb();const tx=db.transaction(store,"readwrite");const idx=tx.objectStore(store).index(indexName);await new Promise((resolve,reject)=>{const cursor=idx.openKeyCursor(IDBKeyRange.only(value));cursor.onerror=()=>reject(cursor.error);cursor.onsuccess=()=>{const cur=cursor.result;if(!cur){resolve();return}tx.objectStore(store).delete(cur.primaryKey);cur.continue()}});await txDone(tx);db.close()}
async function del(store,key){if(hasAndroidLocalStore()&&store===STORES.exams)return androidDelete("exam_sessions",key);if(hasAndroidLocalStore()&&store===STORES.notebooks)return androidDelete("notebooks",key);const db=await openOfflineDb();const tx=db.transaction(store,"readwrite");tx.objectStore(store).delete(key);await txDone(tx);db.close();emit()}

export async function installOfflinePack(pack){
  const incomingSubjects=[...new Set((pack?.banks||[]).map(x=>normalize(x.subject)).filter(Boolean))];
  const oldPack=await get(STORES.meta,"pack");
  const db=await openOfflineDb();
  const tx=db.transaction([STORES.questions,STORES.meta],"readwrite");
  const questions=tx.objectStore(STORES.questions);

  for(const subject of incomingSubjects){const keys=await req(questions.index("subject").getAllKeys(subject));for(const key of keys)questions.delete(key)}
  ...
  const mergedSubjects=[...new Set([...(oldPack?.subjects||[]),...(pack?.banks||[]).map(x=>x.subject)])];
...
async function replaceOfflineSubject(subject,title,questions,version){
  ...
  const oldKeys=await req(store.index("subject").getAllKeys(subject));for(const key of oldKeys)store.delete(key);
  for(const question of questions)store.put({...question,subject,key:`${subject}:${question.id}`});
  tx.objectStore(STORES.meta).put({
    key:"pack",version:version||oldPack?.version||"unknown",downloaded_at:nowIso(),
    subjects:[...new Set([...(oldPack?.subjects||[]),subject])]
  });
  await txDone(tx);db.close();emit();
}

export async function downloadOfflinePack(subjects=[]){
  if(standaloneEnabled()){const pack=bundledContentPack();const wanted=new Set((subjects||[]).map(normalize).filter(Boolean));return installOfflinePack({...pack,banks:pack.banks.filter(b=>!wanted.size||wanted.has(normalize(b.subject)))});}
  const manifestResponse=await fetch("/api/offline/manifest",{cache:"no-store"});
  const manifest=await manifestResponse.json().catch(()=>({}));
  if(!manifestResponse.ok)throw new Error(manifest.error||"Não foi possível listar os bancos offline.");
  ...
  for(const bank of banks){
    ...
    let offset=0;
    do{
      const params=new URLSearchParams({subjects:bank.subject,offset:String(offset),limit:"100"});
      const response=await fetch("/api/offline/bootstrap?"+params.toString(),{cache:"no-store"});
      const payload=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(payload.error||"Não foi possível baixar "+bank.subject+".");
      const chunk=payload.banks?.[0];
      questions.push(...(chunk?.questions||[]));
      offset=chunk?.next_offset;
      await yieldToBrowser();
    }while(offset!==null&&offset!==undefined);
    await replaceOfflineSubject(bank.subject,bank.title,questions,manifest.version);
  ...
  try{
    if("serviceWorker" in navigator){
      ...
      reg.active?.postMessage({
        ...
        routes:["/offline","/area-do-aluno","/hoje","/simulado","/conteudos/banco-de-questoes",...(status.subjects||[]).map(subject=>"/simulado/"+subject)]
      });
    }
  }catch{}
  return status;
...
export async function ensureOfflinePackCurrent({force=false}={}){
  if(standaloneEnabled()){try{await navigator.storage?.persist?.()}catch{}const pack=bundledContentPack();const local=await getOfflineStatus();if(!force&&local.installed&&local.version===pack.version&&Number(local.questions||0)===Number(pack.total_questions||0))return {status:local,updated:false,manifest:pack};const status=await installOfflinePack(pack);return {status,updated:true,manifest:pack};}
  if(typeof navigator!=="undefined"&&!navigator.onLine)return {status:await getOfflineStatus(),updated:false,offline:true};
  try{await navigator.storage?.persist?.()}catch{}
  const [local,manifestResponse]=await Promise.all([
    getOfflineStatus(),
    fetch("/api/offline/manifest",{cache:"no-store"})
  ]);
  const manifest=await manifestResponse.json().catch(()=>({}));
  if(!manifestResponse.ok)throw new Error(manifest.error||"Não foi possível verificar o banco offline.");
  const needsUpdate=
    force||
    !local.installed||
    local.version!==manifest.version||
    Number(local.questions||0)!==Number(manifest.total_questions||0)||
    (manifest.banks||[]).some(bank=>!(local.subjects||[]).includes(bank.subject));
  if(!needsUpdate)return {status:local,updated:false,manifest};
  const status=await downloadOfflinePack();
  return {status,updated:true,manifest};
}
...
async function questionsForSubjects(subjects){
  const wanted=[...new Set((subjects||[]).map(normalize).filter(Boolean))];
  if(!wanted.length)return all(STORES.questions);
  const groups=await Promise.all(wanted.map(subject=>allByIndex(STORES.questions,"subject",subject)));
  return groups.flat();
}
...
function pscppOfflineSelection(pool,count){
  return buildPscppFixedExam(pool,{seed:uuid()});
}
...
export async function queueOfflineEvent(type,payload,{id=null}={}){
  ...
  if(standaloneEnabled())await appendStandaloneChange(type,payload,{id:event.id});
  try{
...
export async function listOfflineNotebooks(){return (await all(STORES.notebooks)).sort((a,b)=>String(b.updated_at||b.created_at).localeCompare(String(a.updated_at||a.created_at)))}

export async function answerOfflineNotebook(notebookId,{subject,question_id,selected_answer,response_time_ms=0,plan_task=null}){
  ...
  const chosen=String(selected_answer||"").toUpperCase();const correct=answerKey(q);const saved={selected_answer:chosen,is_correct:chosen===correct,correct_answer:correct||null,explanation:q.explanation||null,source:q.source||null,response_time_ms:Math.max(0,Math.min(14400000,Math.trunc(Number(response_time_ms)||0))),answered_at:nowIso()};
  notebook.answers={...(notebook.answers||{}),[key]:saved};notebook.updated_at=nowIso();await put(STORES.notebooks,notebook);
  await queueOfflineEvent("notebook.answer",{notebook_id:String(notebook.id),subject,question_id:String(question_id),selected_answer:chosen,response_time_ms:saved.response_time_ms,plan_task},{id:`notebook-answer:${notebook.id}:${subject}:${question_id}`});
...
export async function createOfflineExam({subject,count=100,filters={},title=null,planTask=null}){
  await ensureOfflinePackCurrent();
  ...
  const wanted=Math.min(100,Math.max(1,Number(count)||100));
  const selected=normalize(subject)==="simulado-pscpp"
    ? pscppOfflineSelection(pool,wanted)
    : buildHistoricalExam(pool,{seed:uuid(),size:wanted});
...
export async function cacheServerExam(exam,planTask=null){
  ...
  const restored=Object.fromEntries((exam.questions||[]).filter(q=>q.answer).map(q=>[String(q.id),q.answer]));
  const merged={...(existing||{}),id:String(exam.id),subject:exam.subject,title:exam.title,question_ids:(exam.questions||[]).map(q=>String(q.id)),started_at:exam.started_at,expires_at:exam.expires_at,paused_at:exam.paused_at||null,remaining_seconds:exam.remaining_seconds??null,status:exam.status||"in_progress",answers:{...(existing?.answers||{}),...restored},planTask:planTask||existing?.planTask||null};
  await put(STORES.exams,merged);return merged;
}
export async function getOfflineExam(id){return get(STORES.exams,String(id))}
export async function listOfflineExams(){return (await all(STORES.exams)).sort((a,b)=>String(b.started_at).localeCompare(String(a.started_at)))}
export async function hydrateOfflineExam(exam){
  ...
  const result=!['in_progress','paused'].includes(exam.status)?calcResult(questions.length,answers):null;
  return {
    ...exam,
    questions:questions.map(question=>answers[String(question.id)]?{...question,answer:answers[String(question.id)]}:question),
...
export async function setOfflineExamPaused(examId,action){
  const exam=await getOfflineExam(examId);if(!exam)throw new Error("Simulado offline não encontrado.");
  if(!['in_progress','paused'].includes(exam.status))throw new Error("Este simulado já foi finalizado.");
  if(action==='pause'&&exam.status==='in_progress'){
    exam.remaining_seconds=Math.max(0,Math.floor((new Date(exam.expires_at).getTime()-Date.now())/1000));
    exam.paused_at=nowIso();exam.status='paused';
  }else if(action==='resume'&&exam.status==='paused'){
    exam.expires_at=new Date(Date.now()+Math.max(0,Number(exam.remaining_seconds)||0)*1000).toISOString();
    exam.paused_at=null;exam.remaining_seconds=null;exam.status='in_progress';
  }
  await put(STORES.exams,exam);await queueExamSnapshot(exam);return hydrateOfflineExam(exam);
}
...
export async function syncOfflineQueue(){
  if(standaloneEnabled())return {ok:true,standalone:true,synced:0,pending:(await all(STORES.queue)).length};
  if(typeof navigator!=="undefined"&&!navigator.onLine)return {ok:false,offline:true,synced:0,pending:(await all(STORES.queue)).length};

  let synced=0;
  let failed=0;

  for(let pass=0;pass<20;pass++){
    const events=(await all(STORES.queue))
      .sort((a,b)=>{
        const byTime=String(a.created_at).localeCompare(String(b.created_at));
        return byTime||syncEventPriority(a.type)-syncEventPriority(b.type)||String(a.id).localeCompare(String(b.id));
      })
      .slice(0,500);
    ...
    if(removed===0||events.length<500)break;
  }

  const status=await getOfflineStatus();
  emit();
  return {ok:true,synced,pending:status.pending,failed};
}

export async function getOfflineStatus(){
  // Startup path: count() avoids materializing the entire question bank in JS.
  // This is especially important on iPhone/Safari, where getAll() on thousands
  // of questions can block the main thread and delay the first interactive paint.
  const [pack,questions,pending,notebooks,exams]=await Promise.all([
    get(STORES.meta,"pack"),count(STORES.questions),count(STORES.queue),count(STORES.notebooks),count(STORES.exams)
  ]);
  return {installed:Boolean(pack),version:pack?.version||null,downloaded_at:pack?.downloaded_at||null,subjects:pack?.subjects||[],questions,pending,notebooks,exams};
}
