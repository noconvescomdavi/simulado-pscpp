...
export async function getErrorNotebook(userId, limit = 60) {
  ...
  const result = await query(
    `SELECT
         ...
         qs.error_count,
         qs.last_answered_at,
         last_attempt.selected_answer AS last_selected_answer,
         last_attempt.is_correct AS last_is_correct,
         last_attempt.answered_at AS last_attempt_at,
         last_attempt.response_time_ms AS last_response_time_ms
       FROM question_stats qs
       LEFT JOIN LATERAL (
         SELECT qa.selected_answer, qa.is_correct, qa.answered_at, qa.response_time_ms
           FROM question_answers qa
          WHERE qa.user_id=qs.user_id
            AND qa.subject=qs.subject
            AND qa.question_id=qs.question_id
          ORDER BY qa.answered_at DESC, qa.id DESC
          LIMIT 1
       ) last_attempt ON TRUE
      WHERE qs.user_id=$1
        AND qs.error_count > 0
      ORDER BY qs.error_count DESC, qs.last_answered_at DESC NULLS LAST
      LIMIT $2`,
    [userId, safeLimit]
  );
  ...
  return result.rows.map((row) => {
    ...
    const taxonomy=question?questionTaxonomy(question,slug):{};
    const rawSource=question?.source;
    const sourceReference={
      work:taxonomy?.work?.title||taxonomy?.work?.label||null,
      chapter:taxonomy?.chapter?.label||taxonomy?.chapter?.title||taxonomy?.section||null,
      page:question?.page||question?.page_reference||question?.source_page||(rawSource&&typeof rawSource==="object"?(rawSource.page||rawSource.pages||null):null),
      text:typeof rawSource==="string"?rawSource:(rawSource?.reference||rawSource?.title||null)
    };
    ...
    return {
      ...
      selected_answer: row.last_selected_answer || null,
      correct_answer: question?.correct_answer || null,
      last_attempt_correct: row.last_is_correct === true,
      last_attempt_at: row.last_attempt_at || row.last_answered_at || null,
      last_response_time_ms: row.last_response_time_ms == null ? null : n(row.last_response_time_ms),
      ...
      recovered: row.last_is_correct === true && n(row.error_count) > 0,
      mastery_status:
        pct(n(row.correct_count), n(row.answer_count)) >= 80
          ? "dominado"
          : pct(n(row.correct_count), n(row.answer_count)) >= 60
            ? "em desenvolvimento"
            : "precisa estudar",
    };
  });
...
export async function getTodayStudyPlan(userId) {
  const [stats, goals, completions, todayActivity] = await Promise.all([
    query(
    ...
    query(
      `SELECT count(*)::int AS questions_answered
        ...
        WHERE user_id=$1
          AND (answered_at at time zone 'America/Sao_Paulo')::date =
              (now() at time zone 'America/Sao_Paulo')::date`,
      [userId]
    ).catch(() => ({ rows: [{ questions_answered: 0 }] })),
  ]);
  ...
  const questionsAnsweredToday = n(todayActivity.rows[0]?.questions_answered);
  ...
  const tasks = rawTasks.map((task) => {
    ...
    const automaticallyCompleted =
      task.key === "questions" && questionsAnsweredToday >= dailyQuestionTarget;

    return {
      ...task,
      completed: automaticallyCompleted || saved?.completed === true,
      automatic: automaticallyCompleted,
      completed_at: automaticallyCompleted
...
export async function setStudyGoals(userId, input = {}) {
  const dailyMinutes = Math.max(10, Math.min(720, Math.trunc(Number(input.daily_minutes) || 60)));
  const weeklyQuestions = Math.max(7, Math.min(10000, Math.trunc(Number(input.weekly_questions) || 100)));

  const result = await query(
    `INSERT INTO student_study_goals(user_id,daily_minutes,weekly_questions,updated_at)
     VALUES($1,$2,$3,now())
     ON CONFLICT(user_id) DO UPDATE SET
       daily_minutes=excluded.daily_minutes,
       weekly_questions=excluded.weekly_questions,
       updated_at=now()
     RETURNING daily_minutes,weekly_questions,target_exam_date`,
    [userId, dailyMinutes, weeklyQuestions]
  );
