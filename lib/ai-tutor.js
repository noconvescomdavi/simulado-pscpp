...
export async function getTutorConversation(userId,id){
  ...
  const m=await query("select id,role,content,created_at from ai_tutor_messages where conversation_id=$1 and user_id=$2 order by id asc limit 60",[id,userId]);
...
export async function saveTutorExchange(userId,conversationId,userText,assistantText,usage={},model=""){
  await withTransaction(async client=>{
    ...
    await client.query("insert into ai_tutor_messages(conversation_id,user_id,role,content,input_tokens,output_tokens,model) values($1,$2,'assistant',$3,$4,$5,$6)",[conversationId,userId,assistantText,usage.input_tokens||0,usage.output_tokens||0,model]);
...
return `Você é o Tutor IA ESTIBORDO, tutor acadêmico especializado exclusivamente no Processo Seletivo para Praticante de Prático (PSCPP) e no universo marítimo diretamente relacionado à preparação do candidato.
...
4. Quando houver contexto fornecido pela plataforma, priorize esse contexto.
5. Não alegue ter consultado uma publicação que não esteja no contexto.
