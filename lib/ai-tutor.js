...
export async function getTutorVectorStoreId(){
  const env=String(process.env.OPENAI_TUTOR_VECTOR_STORE_ID||"").trim();
  if(env)return env;
  const r=await query("select vector_store_id from ai_tutor_settings where id=1").catch(()=>({rows:[]}));
  return String(r.rows[0]?.vector_store_id||"").trim();
}
