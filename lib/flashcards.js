...
import { buildAllPscppBibliographyDecks } from "./bibliography-flashcards";
...
let bibliographySyncPromise=null;
async function ensurePscppBibliographyDecks(){
  if(!bibliographySyncPromise){
    bibliographySyncPromise=(async()=>{
      const decks=buildAllPscppBibliographyDecks();
      for(let i=0;i<decks.length;i++){
        const d=decks[i];
        await flashQuery(
          `insert into decks(slug,subject_slug,subject_label,title,description,cards,is_active,sort_order,updated_at)
           values($1,$2,$3,$4,$5,$6::jsonb,true,$7,now())
           on conflict(slug) do update set
             subject_slug=excluded.subject_slug,subject_label=excluded.subject_label,title=excluded.title,
             description=excluded.description,cards=excluded.cards,is_active=true,sort_order=excluded.sort_order,
             updated_at=case when decks.cards is distinct from excluded.cards then now() else decks.updated_at end`,
          [d.slug,d.subject_slug,d.subject_label,d.title,d.description,JSON.stringify(d.cards),20+i]
        );
      }
    })().catch(error=>{bibliographySyncPromise=null;throw error;});
  }
  return bibliographySyncPromise;
}
...
export async function getFlashcardDeck(slug) {
  await ensurePscppBibliographyDecks();
...
export async function listFlashcardDecks(userId) {
  // Listing is read-only. Deck synchronization must never run on a page GET;
  // doing so turns normal navigation into a burst of database writes.
  const result = await flashQuery(
