...
export function isRipeamQuestion(question) {
  const tags = Array.isArray(question?.tags) ? question.tags : [];
  const canonical = question?.taxonomy && typeof question.taxonomy === "object" ? question.taxonomy : {};
  const source = sourceData(question);
  const haystack = normalizeText([
    ...
    question?.topic,
    question?.topic_code,
    canonical.bibliography_id,
    ...tags,
  ].filter(Boolean).join(" "));

  return (
    question?.scenario_only === true ||
    question?.ripeam_only === true ||
    canonical.bibliography_id === "colreg" ||
    tags.includes("ripeam-banco") ||
    tags.includes("ripeam-situacional") ||
    tags.includes("situacao-de-manobra") ||
    haystack.includes("ripeam") ||
    haystack.includes("colreg") ||
    haystack.includes("international regulations for preventing collisions at sea")
  );
}
...
export function questionMatchesFilters(question, rawFilters, fallbackSubject = "") {
  ...
  if (filters.scenario_only && !isRipeamQuestion(question)) return false;
