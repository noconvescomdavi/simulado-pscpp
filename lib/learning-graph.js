...
export async function getLearningGraph(userId){
  ...
  for(const bankInfo of availableQuestionBanks()){
    ...
    // A árvore oficial não pode desaparecer só porque ainda não existe questão indexada.
    for(const unit of requiredUnits){
      const workId=unit.bibliography_key||norm(unit.publication)||"bibliografia-oficial";
      const work=works.get(workId)||{id:workId,title:unit.publication||unit.source||"Bibliografia oficial",questions:0,chapters:new Map()};
      const chapterId=unit.section_key||norm(unit.chapter||unit.section)||"secao-oficial";
      if(!work.chapters.has(chapterId)){
        work.chapters.set(chapterId,{
          id:chapterId,
          label:unit.chapter||unit.section||"Seção prevista no conteúdo programático",
          questions:0,
          topics:new Map(),
          official:true,
  ...
  return{version:"v2-official-coverage",totals,subjects};
}
