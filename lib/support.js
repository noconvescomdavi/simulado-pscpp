...
export async function ensureSupportSchema() {
  ...
  await query("create table if not exists support_tickets (id bigserial primary key, user_id uuid not null references users(id) on delete cascade, subject varchar(140) not null, category varchar(40) not null default 'technical', priority varchar(20) not null default 'normal', status varchar(30) not null default 'open', created_at timestamptz not null default now(), updated_at timestamptz not null default now(), last_message_at timestamptz not null default now(), student_unread integer not null default 0, admin_unread integer not null default 1)");
  await query("create table if not exists support_messages (id bigserial primary key, ticket_id bigint not null references support_tickets(id) on delete cascade, sender_id uuid not null references users(id) on delete cascade, sender_role varchar(20) not null, body text not null, created_at timestamptz not null default now())");
