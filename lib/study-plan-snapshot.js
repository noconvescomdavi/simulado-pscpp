...
export async function persistStudyPlanSnapshot(userId,plan){
  ...
  return withTransaction(async(client)=>{
    ...
    const snapshot=await client.query(
      "insert into student_plan_snapshots(user_id,week_start,week_end,algorithm_version,source,metadata) values($1,$2::date,$3::date,'v4.2-coverage-fixation','weekly_plan',$4::jsonb) returning *",
      [userId,weekStart,weekEnd,JSON.stringify({phase:plan.phase?.key||null,days_left:plan.days_left??null,bibliography_completion_deadline:"2027-07-30",heavy_review_start:"2027-07-31",heavy_review_end:"2027-10-31",target_exam_date:"2027-11-01",daily_questions_mode:"same_day_reading_fixation",question_match:"bibliography_key_section_key",friday_rule:"weekly_error_review",error_window:"saturday_thursday",recurring_errors_priority:true})]
    );
