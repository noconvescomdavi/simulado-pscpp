...
export async function persistStudyPlanSnapshot(userId,plan){
  ...
  return withTransaction(async(client)=>{
    ...
    if(existing.rows[0]?.algorithm_version==="v5.0-four-daily-tasks")return {snapshot:existing.rows[0],created:false};
    if(existing.rows[0]){
      await client.query("delete from student_plan_snapshot_tasks where snapshot_id=$1",[existing.rows[0].id]);
      await client.query("delete from student_plan_snapshots where id=$1",[existing.rows[0].id]);
    }

    const snapshot=await client.query(
      "insert into student_plan_snapshots(user_id,week_start,week_end,algorithm_version,source,metadata) values($1,$2::date,$3::date,'v5.0-four-daily-tasks','weekly_plan',$4::jsonb) returning *",
      [userId,weekStart,weekEnd,JSON.stringify({phase:plan.phase?.key||null,days_left:plan.days_left??null,bibliography_completion_deadline:"2027-08-01",heavy_review_start:"2027-08-02",heavy_review_end:"2027-10-31",target_exam_date:"2027-11-01",daily_questions_mode:"same_day_reading_fixation",question_match:"bibliography_key_section_key",daily_tasks:["reading","questions","review"],sunday_task:"simulado",question_match:"bibliography_key_section_key",review_mode:"commented_errors_with_bibliographic_reference",recurring_errors_priority:true})]
    );
...
export async function loadStudyPlanSnapshot(userId,weekStart){
  ...
  const r=await withTransaction(async(client)=>{
    ...
    if(!snapshot||snapshot.algorithm_version!=="v5.0-four-daily-tasks")return null;
