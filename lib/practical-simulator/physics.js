...
export function stepVessel(s,ship,dt=1){
 const target=ENGINE_TARGETS[s.engine]??0, accel=(target-s.stw)*.018;
 const stw=s.stw+accel*dt, turnGain=Math.max(.15,Math.min(1.15,Math.abs(stw)/8));
 const rot=(s.rudder/35)*2.1*turnGain, hdg=norm360(s.hdg+rot*dt/60);
