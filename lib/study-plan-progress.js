...
async function auxiliaryQuery(client,sql,params=[]){
  await client.query("SAVEPOINT study_plan_aux");
  try{
    const result=await client.query(sql,params);
    await client.query("RELEASE SAVEPOINT study_plan_aux");
    return result;
  }catch(error){
    await client.query("ROLLBACK TO SAVEPOINT study_plan_aux");
    await client.query("RELEASE SAVEPOINT study_plan_aux");
    console.error("Falha não bloqueante em gravação auxiliar do plano:",error);
    return null;
  }
}
...
export async function setPlanTaskStatusAndMetrics(userId,input={}){
  ...
  return withTransaction(async(client)=>{
    ...
    // Não depende de ON CONFLICT/PK do banco. Algumas instalações antigas
    // podem ter a tabela criada antes da constraint composta atual.
    // O advisory lock acima serializa usuário+tarefa+data, então UPDATE-first
    // + INSERT-if-missing permanece idempotente inclusive sob clique duplo.
    stage="update_progress";
    let saved=await client.query(`
      update student_plan_task_progress
      set task_type=$4::varchar,
          subject_slug=$5::varchar,
          status=$6::varchar,
          completed_at=case
            when $6::varchar='done' then coalesce(completed_at,now())
            else null
          end,
          metadata=$7::jsonb,
          updated_at=now()
    ...
    if(!saved.rowCount){
      ...
      saved=await client.query(`
        insert into student_plan_task_progress(
          ...
          $1,$2,$3::date,$4::varchar,$5::varchar,$6::varchar,case when $6::varchar='done' then now() else null end,$7::jsonb,now()
        )
        returning *
      `,[userId,taskKey,planDate,taskType,subjectSlug,status,JSON.stringify(input.metadata||{})]);
    }
