...
async function recordStudyActivity(client,userId,{eventName,subjectSlug,metadata={}}){
  ...
  await client.query(`
    insert into student_study_sessions(
      user_id,session_type,subject_slug,started_at,ended_at,duration_seconds,metadata
    ) values($1,'plan_task',$2,now(),now(),0,$3::jsonb)
  `,[userId,subjectSlug||null,JSON.stringify(metadata||{})]);
}
...
async function applyReadingChunk(client,userId,input){
  ...
  let pagesCompleted=Math.max(0,Number(current?.pages_completed||0)+addedPages);
  if(totalPages)pagesCompleted=Math.min(totalPages,pagesCompleted);
  const done=totalPages?pagesCompleted>=totalPages:pages===0;
  const status=done?"done":"reading";
  const ranges=Array.isArray(current?.completed_ranges)?current.completed_ranges:[];
  const nextRanges=addedPages?[...ranges,{from:pageFrom,to:pageTo}]:ranges;
...
export async function setPlanTaskStatusAndMetrics(userId,input={}){
  ...
  return withTransaction(async(client)=>{
    ...
    if(status==="done"){
      await client.query(
        "update student_plan_reschedules set status='completed',completed_at=coalesce(completed_at,now()) where user_id=$1 and task_key=$2 and original_plan_date=$3::date and status='active'",
        [userId,taskKey,planDate]
      ).catch(()=>{});
    }
