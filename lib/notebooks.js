...
export async function createNotebook(
  ...
  // Priorize questões ainda não respondidas para ampliar cobertura antes de repetir itens.
  const answered=await query(`select subject,question_id,answer_count from question_stats where user_id=$1 and answer_count>0`,[userId]).catch(()=>({rows:[]}));
  const answeredSet=new Set(answered.rows.map(x=>normalizeSubject(x.subject)+":"+String(x.question_id)));
  const fresh=pool.filter(ref=>!answeredSet.has(ref.subject+":"+ref.id));
  const repeated=pool.filter(ref=>answeredSet.has(ref.subject+":"+ref.id));
  const refs=[...shuffle(fresh),...shuffle(repeated)].slice(0,Math.min(requestedCount,pool.length));
...
function notebookScope(refs, fallbackSubjects=[]){
  ...
  return {subjects:[...subjectSet].map(subjectLabel),works:[...works.values()],chapters:[...chapters.values()]};
}
...
export async function answerNotebook({
  ...
  return withTransaction(async (client) => {
    ...
    const safeResponseTimeMs = Math.max(0, Math.min(14_400_000, Math.trunc(Number(responseTimeMs) || 0)));
