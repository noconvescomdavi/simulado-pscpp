...
function fixationScore(q,fixation){
  ...
  const hay=norm([q.topic_code,q.topic,q.module,...(q.tags||[])].join(" "));
  const chapter=norm(fixation.chapter||"");
  const tokens=chapter.replace(/capitulo|chapter|section|secao|volume|parte|regulation|annex/g," ").split(/[^a-z0-9]+/).filter(x=>x.length>2);
...
export async function createNotebook(
  userId,
  { subjects, count, fixation=null, title=null }
) {
  ...
  const result = await query(
    ...
    [
      userId,
      title ? String(title).slice(0,180) : `Caderno de ${refs.length} questões`,
