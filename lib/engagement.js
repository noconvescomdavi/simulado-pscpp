...
import {calculateReviewSchedule} from "./learning-algorithms";
...
export async function completeReview(userId,sourceKey,quality="good"){
  ...
  const schedule=calculateReviewSchedule(quality,stability);
  const {lapse,days,stabilityDelta,difficultyDelta}=schedule;
...
export async function getConsistency(userId){
  ...
  const set=new Set(days.rows.map(r=>r.study_date instanceof Date?r.study_date.toISOString().slice(0,10):String(r.study_date).slice(0,10)));
  ...
  for(let i=0;i<365;i++){
    ...
    const k=new Intl.DateTimeFormat("en-CA",{timeZone:"America/Sao_Paulo",year:"numeric",month:"2-digit",day:"2-digit"}).format(d);
