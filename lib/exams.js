...
export async function finalizeExamSession(client, session, requestedReason = "manual") {
  ...
  const remainingAtFinish = session.status === "paused"
    ? Number(session.remaining_seconds || 0)
    : Math.max(0, Math.round((new Date(session.expires_at).getTime() - Date.now()) / 1000));
  const durationSeconds = Math.max(
    0,
    Math.min(EXAM_DURATION_SECONDS, EXAM_DURATION_SECONDS - remainingAtFinish)
  );
...
export async function getExamState(userId, rawSubject) {
  ...
  return withTransaction(async (client) => {
    const current = await client.query(
      `select *, status='in_progress' and expires_at <= now() as is_expired
         from exam_sessions
        where user_id=$1 and subject=$2 and status in ('in_progress','paused')
    ...
    if (session) {
      ...
      const answers = await sessionAnswers(client, session.id);
      return { state: session.status, exam: activePayload(session, answers), resumed: answers.length > 0 };
    }
...
export async function startExam(userId, rawSubject, { filters } = {}) {
  const subject = normalizeSubject(rawSubject);
  const normalizedFilters = normalizeQuestionFilters(filters);
  const bank = filteredQuestionBank(subject, normalizedFilters);

  if (!bank?.questions?.length) {
    return {
      error: Object.values(normalizedFilters).some(Boolean)
        ? "Nenhuma questão corresponde aos filtros selecionados."
        : "Matéria sem banco de questões.",
  ...
  return withTransaction(async (client) => {
    ...
    if (existing && !existing.is_expired) {
      const answers = await sessionAnswers(client, existing.id);
      return { state: existing.status, exam: activePayload(existing, answers), resumed: true };
    }
    ...
    const requestedSize = subject === "teste-gratuito" ? 10 : EXAM_SIZE;
    const ids = subject === PSCPP_SUBJECT_SLUG
      ? buildPscppExam(`${userId}:${Date.now()}`).map((question) => String(question.id))
      : buildHistoricalExam(bank.questions, {
          seed: `${userId}:${subject}:${Date.now()}`,
          size: Math.min(requestedSize, bank.questions.length),
...
export async function submitExamAnswer({
  ...
  return withTransaction(async (client) => {
    ...
    if (session.status === "paused") {
      return {
        error: "Continue o simulado antes de responder.",
        code: "EXAM_PAUSED",
        status: 409,
      };
    }
    ...
    const isPscpp = subject === PSCPP_SUBJECT_SLUG;
    const metricSubject = isPscpp ? PSCPP_SUBJECT_SLUG : normalizeSubject(question?.source_subject || subject);
    const metricQuestionId = isPscpp ? String(question?.id || questionId) : String(question?.source_id || question?.id || questionId);
    const ids = questionIds(session.question_ids);
...
export async function setExamPaused(userId, rawSubject, sessionId, action) {
  const subject = normalizeSubject(rawSubject);
  ...
  return withTransaction(async (client) => {
    ...
    const session = row.rows[0];
    if (!session) return { error: "Simulado não encontrado.", code: "EXAM_NOT_FOUND", status: 404 };
    if (!["in_progress", "paused"].includes(session.status)) {
      return { error: "Este simulado já foi finalizado.", code: "EXAM_FINISHED", status: 409 };
    }
    // Uma sessão já pausada não deve expirar enquanto permanece pausada.
    // is_expired só é significativo para o estado in_progress.
    if (session.is_expired && session.status === "in_progress") {
      const result = await finalizeExamSession(client, session, "timeout");
      return { error: "O tempo do simulado terminou.", code: "EXAM_EXPIRED", status: 409, result };
    }

    if ((shouldPause && session.status === "paused") || (!shouldPause && session.status === "in_progress")) {
      const answers = await sessionAnswers(client, session.id);
      return { ok: true, state: session.status, exam: activePayload(session, answers) };
    }
      ...
      ? await client.query(
          `update exam_sessions
              set status='paused', paused_at=now(),
                  remaining_seconds=greatest(0,least($2::int,floor(extract(epoch from (expires_at-now())))::int)),
                  updated_at=now()
            where id=$1 returning *`,
          [session.id, EXAM_DURATION_SECONDS]
        )
      : await client.query(
          `update exam_sessions
              set status='in_progress', paused_at=null,
                  expires_at=now()+(coalesce(remaining_seconds,0)*interval '1 second'),
                  remaining_seconds=null, updated_at=now()
            where id=$1 returning *`,
          [session.id]
        );
    const current = updated.rows[0];
    const answers = await sessionAnswers(client, current.id);
    return { ok: true, state: current.status, exam: activePayload(current, answers) };
  });
}
