...
function scorePercent(correct, total) {
  return total ? Math.round((correct / total) * 10000) / 100 : 0;
}

function sessionResult(session) {
  const answered = Number(session.answered_count || 0);
  const correct = Number(session.correct_count || 0);
  const total = Number(session.total_questions || questionIds(session.question_ids).length);
  ...
  if (!attemptId) {
    const attempt = await client.query(
      ...
      [
        ...
        session.subject,
        scorePercent(correct, totalQuestions),
        correct,
