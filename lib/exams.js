...
export async function getExamState(userId, rawSubject) {
  ...
  return withTransaction(async (client) => {
    ...
    if (session) {
      ...
      return { state: "in_progress", exam: activePayload(session, answers), resumed: answers.length > 0 };
    }
...
export async function startExam(userId, rawSubject, { filters } = {}) {
  ...
  return withTransaction(async (client) => {
    ...
    if (existing && !existing.is_expired) {
      ...
      return { state: "in_progress", exam: activePayload(existing, answers), resumed: true };
    }
