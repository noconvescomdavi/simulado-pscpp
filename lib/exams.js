...
export async function finalizeExamSession(client, session, requestedReason = "manual") {
  ...
  if (status === "completed") {
    await client.query(
      `insert into student_plan_task_progress(user_id,task_key,plan_date,task_type,subject_slug,status,completed_at,metadata,updated_at)
       values($1,'weekly-exam|' || to_char((now() at time zone 'America/Sao_Paulo')::date,'YYYY-MM-DD'),(now() at time zone 'America/Sao_Paulo')::date,'simulado',$2,'done',now(),$3::jsonb,now())
       on conflict(user_id,task_key,plan_date) do update set status='done',completed_at=now(),metadata=excluded.metadata,updated_at=now()`,
      [session.user_id, session.subject, JSON.stringify({source:"automatic_exam_completion",session_id:session.id,reason})]
    ).catch(()=>{});
  }
