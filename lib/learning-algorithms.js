...
// Confiança amostral conservadora: 20 acertos excelentes ainda não equivalem
// a domínio consolidado. ~100 respostas atingem confiança máxima.
export function calculateSampleConfidence(answers){
  const total=Math.max(0,n(answers));
  if(total===0)return 0;
  return Math.min(100,Math.sqrt(total/100)*100);
}
...
export function calculateMasteryScore({answers,correct,last_answered_at,stability,review_count,lapse_count},nowMs=Date.now()){
  ...
  const confidence=calculateSampleConfidence(total);
  ...
  // Acurácia só recebe peso integral quando há amostra suficiente.
  // Sem respostas, o estado é explicitamente não mensurado, nunca 0% de conhecimento.
  const score=total===0?0:
    accuracy*(.40+.15*evidenceFactor)+
    confidence*.20+
    recency*.10+
    reviewStrength*.15-
    lapsePenalty;
  return{
    ...
    evidence_state:total===0?"unmeasured":confidence<50?"low_sample":confidence<80?"developing_sample":"reliable_sample",
    measured:total>0
  };
}
