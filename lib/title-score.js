...
const COMMAND_ELIGIBLE_CODES = new Set(["CLC","CCB","1ON","2ON","CFL","MCB","PLF","PAP","PRT"]);
...
function categoryCode(value) {
  ...
  for (const code of AQUAVIARIO_POINTS.keys()) {
    if (text === code || text.endsWith(`— ${code}`) || text.endsWith(` ${code}`)) return code;
  }
  return "";
}
...
export function categoryPoints(value, occupationType = "") {
  ...
  return AQUAVIARIO_POINTS.get(categoryCode(value)) || 0;
}
...
export function calculateTitleScore({ category, occupationType, embarkationDays, commandDays }) {
  ...
  const commandEligible = occupationType === "militar_mb" ||
    (occupationType === "aquaviario" && COMMAND_ELIGIBLE_CODES.has(code));
  const command = commandEligible ? commandPoints(commandDays) : 0;
  return {
    embarkation,
    category: professionalCategory,
    command,
    command_eligible: commandEligible,
