...
const SOURCE_LOCATOR_IN_STEM = /(?:\b(?:item|subitem|se[cç][aã]o|cap[ií]tulo|regra|artigo)\s*[\dIVXLCDM]+(?:[.\-][\w]+)*|\bp(?:[aá]g(?:ina)?\.?|g\.)?\s*\d+|\btrecho\s*\d+|arquivo\s+fornecido)/i;
const NEGATIVE_STEM = /\b(?:incorreta|incorreto|n[aã]o\s+(?:est[aá]|corresponde|representa|se\s+aplica|constitui)|exceto)\b/i;
...
function subjectForStem(question = {}) {
  const raw = question.topic || question.taxonomy?.topic || question.module || question.tracking?.topic || question.tracking?.module;
  const clean = String(raw || "o assunto abordado")
    .replace(/\b(?:item|subitem|se[cç][aã]o|cap[ií]tulo|regra|artigo)\s*[\dIVXLCDM]+(?:[.\-][\w]+)*/gi, "")
...
function canonicalSourceStem(question = {}, title, subject) {
  if (NEGATIVE_STEM.test(String(question.question || ""))) {
    return `De acordo com ${title}, acerca de ${subject}, assinale a alternativa INCORRETA.`;
  }
  return `De acordo com ${title}, acerca de ${subject}, assinale a alternativa que apresenta corretamente a disposição, requisito ou conceito previsto na publicação.`;
}
...
export function reformulateLeakingStem(question={}) {
 ...
 const prompt=locatorDependent
   ? canonicalSourceStem(question,title,subject)
   : style.includes("aplic")
     ? `Em uma situação que exija a aplicação dos conceitos de ${subject} previstos em ${title}, qual alternativa apresenta a conduta, requisito ou conceito compatível com a publicação?`
     : style.includes("sequ")
       ? `Considerando os requisitos sobre ${subject} estabelecidos em ${title}, qual alternativa apresenta corretamente uma das disposições aplicáveis?`
       : canonicalSourceStem(question,title,subject);
 return{...question,question:prompt,provenance:{...(question.provenance||{}),mass_reformulation:{version:"source-topic-v2",reason:locatorDependent?"source locator removed from student-facing stem":"stem normalized to remove answer-disclosing source fragment"}}};
}
