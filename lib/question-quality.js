...
export function reformulateLeakingStem(question={}){
 ...
 const parsedArtifact=/<\s*parsed text for page|parsed text for page|arquivo fornecido/i.test(stem);
 const leaking=quoted||artificialLocator||parsedArtifact||(correct&&(continuationRisk(stem,correct)||overlap(stem,correct)>=.58));
 if(!leaking)return question;
 const title=question.source?.title||question.tracking?.work?.title||"a publicação indicada";
 const locator=question.source?.locator||question.tracking?.section||"o trecho indicado";
 ...
 const prompt=negative
   ? `De acordo com ${title}, considerando ${locator}, assinale a alternativa INCORRETA/EXCETO, conforme solicitado no enunciado original.`
   : basePrompt;
