...
export async function createSession(user) {
  ...
  const token = await new SignJWT({ email: user.email, role: user.role || "student", sv: version })
...
export async function getSession() {
  try {
    ...
    const result = await query(
      "select id,email,role,status,session_version from users where id=$1 limit 1",
      [payload.sub]
    );
