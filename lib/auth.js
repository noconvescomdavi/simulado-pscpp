...
export async function createMobileToken(user) {
  const days = sessionDays();
  const version = Number(user.session_version || 1);
  return new SignJWT({ email:user.email, role:user.role||"student", sv:version, aud:"estibordo-android" })
    .setProtectedHeader({alg:"HS256"}).setSubject(String(user.id)).setIssuedAt().setExpirationTime(`${days}d`).sign(secret());
}
...
export async function getMobileSession(request) {
  try {
    ...
    const {payload}=await jwtVerify(auth.slice(7),secret(),{audience:"estibordo-android"});
    ...
    if(!user||user.status!=="active"||Number(payload.sv||1)!==Number(user.session_version||1))return null;
    return {id:String(user.id),email:user.email,role:user.role,adminMfaEnabled:Boolean(user.admin_mfa_enabled),adminMfaVerified:false};
  } catch { return null; }
}
...
export async function getSession() {
  try {
    ...
    const result = await query(
      "select id,email,role,status,session_version,admin_mfa_enabled from users where id=$1 limit 1",
      [payload.sub]
    );
    const user = result.rows[0];
    ...
    return {
      ...
      email: user.email,
      role: user.role,
      adminMfaEnabled: Boolean(user.admin_mfa_enabled),
      adminMfaVerified: Boolean(payload.amfa)
    };
  } catch {
    return null;
  }
