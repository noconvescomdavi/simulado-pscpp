...
import { getPlatformPricing } from "./pricing";

const API_BASE = "https://api.mercadopago.com";
const DEFAULT_ANNUAL_PRICE_CENTS = 259900;
...
export function getPaymentConfig() {
  ...
  const priceCents = DEFAULT_ANNUAL_PRICE_CENTS;
  ...
  return {
    ...
    ready: Boolean(accessToken && webhookSecret && appUrl && priceCents),
  };
}
...
export function buildPreference({ orderId, email, priceCents }) {
    ...
    items: [
      {
        ...
        quantity: 1,
        currency_id: "BRL",
        unit_price: (priceCents || config.priceCents) / 100,
      },
    ],
    payer: { email },
...
export function getTutorPaymentConfig() {
  ...
  return {...base, priceCents:10000, title:"ESTIBORDO — CONTRAMESTRE — 30 dias"};
}

export async function getResolvedPaymentConfig() {
  const pricing = await getPlatformPricing();
  return {...getPaymentConfig(), priceCents: pricing.subscriptionPriceCents};
}

export async function getResolvedTutorPaymentConfig() {
  const pricing = await getPlatformPricing();
  return {...getTutorPaymentConfig(), priceCents: pricing.contramestrePriceCents};
}

export function buildTutorPreference({orderId,email,priceCents}) {
  const config=getTutorPaymentConfig();
  return {
    items:[{id:"estibordo-tutor-ia-30",title:config.title,description:"Pacote adicional CONTRAMESTRE ESTIBORDO por 30 dias.",quantity:1,currency_id:"BRL",unit_price:(priceCents||config.priceCents)/100}],
    payer:{email},external_reference:orderId,statement_descriptor:config.statementDescriptor,
    payment_methods:{installments:1},
    back_urls:{success:`${config.appUrl}/contramestre?retorno=sucesso`,pending:`${config.appUrl}/contramestre?retorno=pendente`,failure:`${config.appUrl}/contramestre?retorno=falha`},
