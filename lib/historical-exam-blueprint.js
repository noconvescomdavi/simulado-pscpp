...
export function isSituationalQuestion(question = {}) {
  const format = normalized(question?.pscpp_format || question?.format || question?.style);
  return ["situation", "situational", "operational", "application", "applied", "aplicada", "situacao", "situacional"].includes(format);
}

export function buildHistoricalExam(pool, { seed = Date.now(), size = 100, quotas: quotaOverride = null, situationalQuotas = null } = {}) {
  ...
  const quotas = quotaOverride || historicalQuotas(target);
  const selected = [];
  const used = new Set();

  for (const [subject, quota] of Object.entries(quotas)) {
    const subjectPool = valid.filter((item) => historicalSubject(item) === subject);
    const situationalTarget = Math.min(Number(situationalQuotas?.[subject] || 0), quota);
    const situational = ranked(subjectPool.filter(isSituationalQuestion), `${seed}:${subject}:situational`).slice(0, situationalTarget);

    for (const question of situational) {
      selected.push(question);
      used.add(String(question.id));
    }

    const remaining = Math.max(0, quota - situational.length);
    for (const question of ranked(subjectPool.filter((item) => !used.has(String(item.id))), `${seed}:${subject}:fill`).slice(0, remaining)) {
      selected.push(question);
      used.add(String(question.id));
    }
  }
