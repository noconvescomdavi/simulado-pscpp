export const BLUEPRINT_VERSION = "2026.09.70";
export const PSCPP_EXAM_SIZE = 70;
export const CURRENT_PROGRAM_NEW_CONTENT_SHARE = 0.10;
...
export const HISTORICAL_SUBJECT_COUNTS = Object.freeze({
  manobrabilidade: 47,
  "arte-naval": 70,
  "navegacao-aguas-restritas": 37,
  "legislacao-regulamentacao": 63,
  "meteorologia-oceanografia": 21,
  comunicacoes: 16,
  "conhecimentos-gerais": 0,
});

// Regra do Simulado PSCPP atual:
// 1) 90% das vagas preservam a proporção observada nas 254 questões oficiais válidas;
// 2) 10% ficam reservadas à Área VII, criada no programa atual;
// 3) o arredondamento usa maiores restos, garantindo exatamente 70 questões.
// Para 70 questões: 12 / 17 / 9 / 16 / 5 / 4 / 7.
export const PSCPP_70_SUBJECT_QUOTAS = Object.freeze({
  manobrabilidade: 12,
  "arte-naval": 17,
  "navegacao-aguas-restritas": 9,
  "legislacao-regulamentacao": 16,
...
export function buildHistoricalExam(pool, { seed = Date.now(), size = 100, quotas: suppliedQuotas = null } = {}) {
  ...
  const quotas = suppliedQuotas || historicalQuotas(target);
  const selected = [];
  const used = new Set();
...
export function pscpp70Blueprint() {
  return {
    version: BLUEPRINT_VERSION,
    size: PSCPP_EXAM_SIZE,
    selection: "historical_incidence_stratified_current_program",
    historical_sample: HISTORICAL_SAMPLE,
    historical_subject_counts: HISTORICAL_SUBJECT_COUNTS,
    current_program_new_content_share: CURRENT_PROGRAM_NEW_CONTENT_SHARE,
    target_quotas: PSCPP_70_SUBJECT_QUOTAS,
    target_percentages: Object.fromEntries(
      Object.entries(PSCPP_70_SUBJECT_QUOTAS).map(([subject, quota]) => [
        subject,
        Number((quota * 100 / PSCPP_EXAM_SIZE).toFixed(2)),
      ])
    ),
    rules: [
      "As 254 questões oficiais válidas definem a incidência histórica.",
      "Questões anuladas não participam da métrica nem do sorteio normal.",
      "Questões inéditas preenchem as vagas do blueprint e não alteram a incidência histórica.",
      "A Área VII recebe 10% das vagas por ser conteúdo programático atual sem série histórica própria.",
      "O sorteio ocorre somente depois de definidas as quotas.",
    ],
  };
}
