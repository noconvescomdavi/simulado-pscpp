...
export const PSCPP_FIXED_QUOTAS = Object.freeze({
  manobrabilidade: 25,
...
function examNorm(value) {
  return String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
    .replace(/[^a-z0-9]+/g, " ").replace(/\s+/g, " ").trim();
}
function conceptLabel(question) {
  const explicit = question?.taxonomy?.term || question?.taxonomy?.concept || question?.tracking?.term?.label ||
    question?.tracking?.term || question?.concept || question?.term;
  if (explicit) return examNorm(explicit);
  const topic = examNorm(question?.topic || question?.topic_code);
  const tags = (question?.tags || []).map(examNorm).filter(Boolean)
    .filter(tag => !["pscpp","arte naval","manobrabilidade","comunicacoes"].includes(tag) && !/^batch \d+$/.test(tag));
  return topic || tags[tags.length - 1] || examNorm(question?.question).split(" ").slice(0, 10).join(" ");
}
function semanticTokens(question) {
  const stop = new Set(["qual","quais","assinale","alternativa","opcao","correta","incorreta","considerando","relacao","sobre","durante","abaixo","acima","segundo","afirmacao","afirmativas","verdadeira","verdadeiras","navio"]);
  return new Set(examNorm([question?.question, question?.topic, ...(question?.tags || [])].join(" "))
    .split(" ").filter(token => token.length >= 4 && !stop.has(token)));
}
function semanticSimilarity(a, b) {
  const A=semanticTokens(a), B=semanticTokens(b); if(!A.size||!B.size)return 0;
  let common=0; for(const token of A)if(B.has(token))common++;
  return common / Math.min(A.size,B.size);
}
function materiallyDuplicates(candidate, selected) {
  const concept=conceptLabel(candidate);
  const stem=examNorm(candidate?.question);
  return selected.some(existing => {
    if(String(existing.id)===String(candidate.id))return true;
    if(stem && stem===examNorm(existing?.question))return true;
    const otherConcept=conceptLabel(existing);
    if(concept && otherConcept && concept===otherConcept)return true;
    return semanticSimilarity(candidate, existing) >= 0.78;
  });
}
export function buildDiversePscppExam(pool, { seed = Date.now() } = {}) {
  const valid=(pool||[]).filter(q=>q&&q.active!==false&&!q.annulled&&q.validation_status!=="rejected");
  const selected=[];
  for(const [subject,quota] of Object.entries(PSCPP_FIXED_QUOTAS)){
    const candidates=ranked(valid.filter(q=>historicalSubject(q)===subject), `${seed}:pscpp:${subject}`);
    const chosen=[];
    for(const q of candidates){
      if(materiallyDuplicates(q,chosen))continue;
      chosen.push(q);
      if(chosen.length===quota)break;
    }
    if(chosen.length!==quota){
      const error=new Error(`Banco PSCPP sem diversidade suficiente em ${subject}: necessárias ${quota}, disponíveis ${chosen.length} sem duplicidade conceitual.`);
      error.code="PSCPP_DIVERSITY_SHORTAGE";
      error.subject=subject; error.required=quota; error.available=chosen.length;
      throw error;
    }
    selected.push(...chosen);
  }
  if(selected.length!==100)throw new Error(`Blueprint PSCPP inválido: ${selected.length}/100 questões selecionadas.`);
  return ranked(selected, `${seed}:pscpp:final`);
}
