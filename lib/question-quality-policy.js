...
export function questionQualityState(question) {
  ...
  if(/<\s*PARSED TEXT FOR PAGE|PARSED TEXT FOR PAGE|\[object Object\]|\b(?:cacacarga|gragragrande|popopor|uu+um|tututubo|descacacarga|lugapopor|quadrangulapopor|ofjloatation)\b|\(art\.\s*\d+(?:\.\d+)?\}|\{[A-Z]\)/i.test(stem+" "+optionBlob))return {active:false,reason:"parsing-artifact"};
