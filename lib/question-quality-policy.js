...
export function questionQualityState(question) {
  ...
  if(options.length!==5)return {active:false,reason:"option-count-"+options.length};
  ...
  if(new Set(normalized).size!==normalized.length)return {active:false,reason:"duplicate-options"};
  ...
  const wordset=t=>new Set(String(t).toLowerCase().normalize("NFD").replace(/[\\u0300-\\u036f]/g,"").replace(/[^a-z0-9 ]/g," ").split(/\\s+/).filter(w=>w.length>=4));
  ...
  const answer=String(question.correct_answer||question.answer||"").trim().toUpperCase(),ci=/^[A-E]$/.test(answer)?answer.charCodeAt(0)-65:-1;
  if(ci>=0&&texts[ci]){const cross=texts.map((t,i)=>i===ci?1:sim(texts[ci],t));if(cross.filter((v,i)=>i!==ci&&v<.08).length>=3)return {active:false,reason:"heterogeneous-distractors"};const lens=texts.map(t=>wordset(t).size).sort((a,b)=>a-b),median=lens[Math.floor(lens.length/2)]||1;if(wordset(texts[ci]).size>Math.max(median*1.8,median+10))return {active:false,reason:"answer-length-cue"};}
