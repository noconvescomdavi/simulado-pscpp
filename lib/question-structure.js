...
export function classifyQuestionStructure(question) {
  ...
  const options = Array.isArray(question?.options) ? question.options.map((option) => clean(option)) : [];
  ...
  if (columns) {
    return {
      ...
      options: options.map(normalizeSequence),
    };
  }
  ...
  return {
    type: structuredHint ? "structured_legacy" : "simple",
    confidence: structuredHint ? "ambiguous" : "high",
    blocks: [{ type: "stem", text }],
    options: options.map(normalizeSequence),
  };
}
