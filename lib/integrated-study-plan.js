...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  const [onboarding,metrics,bp,readingLog,unavailability]=await Promise.all([
  ...
  for(const day of dayObjects){
    day.tasks=day.tasks.map(task=>{
      ...
      return{...task,status:readingDone?"done":savedTask?.status||"pending",completed_at:readingDone?bpItem?.completed_at:savedTask?.completed_at||null,estimate_minutes:estimateTaskMinutes(task)};
    });
  }
