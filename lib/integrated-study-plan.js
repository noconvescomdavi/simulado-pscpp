...
export async function saveOnboarding(userId,input={}){
  ...
  if(pages>0&&status==="done"){
    await query(
      ...
      [userId,String(input.bibliography_key||"").slice(0,180),String(input.subject_slug||"").slice(0,120),String(input.section_key||"").slice(0,180),pageFrom,pageTo,pages]
    );
  }
  return r.rows[0];
}
...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  ...
  const readingDaysLeft=Math.max(1,Math.ceil((internalTarget-today)/DAY/7)*activeStudyDaysPerWeek);
  ...
  const marginDays=projectedFinish?Math.floor((firstPassDeadline-projectedFinish)/DAY):null;
  ...
  return{
    ...
    first_pass:{
      ...
      projected_finish:projectedFinish?isoDate(projectedFinish):null,margin_days:marginDays,
      on_track:marginDays===null?today<=internalTarget:marginDays>=0,
