...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  ...
  const weighted=metrics.subjects.map(m=>{
    ...
    const masteryPenalty=mastery?.topics?Math.max(0,70-Number(mastery.mastery_score||0)):12;
    return {...m,mastery_score:Number(mastery?.mastery_score||0),critical_topics:Number(mastery?.critical_topics||0),weight:subjectWeight(m,onboarding,studied)+masteryPenalty};
