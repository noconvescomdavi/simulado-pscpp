...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  ...
  const history=await query(
    "select task_key,plan_date,task_type,subject_slug,status,completed_at,metadata from student_plan_task_progress where user_id=$1 and plan_date < $2::date and plan_date >= ($2::date - interval '30 days') order by plan_date desc,updated_at desc",
    [userId,todayIso]
  ).catch(()=>({rows:[]}));
  ...
  const pastPlanned=historical.length;
  const pastDone=historical.filter(x=>x.status==="done").length;
