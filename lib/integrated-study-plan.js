...
import {getLearningProfile,getStudyTimeSummary,explainRecommendation} from "./learning-engine";
...
function readingChunks(item,completedPages=0,targetMinutes=45){
  const total=pageCount(item);
  if(!total)return [{...item,chunk_key:"section",page_from:null,page_to:null,pages:null,planned_minutes:Math.max(20,Math.min(60,targetMinutes))}];
  const start=item.page_start+Math.max(0,Number(completedPages||0));
  if(start>item.page_end)return [];
  const density=densityOf(item);
  const pagesPerHour=pagesPerHourFor(density);
  const chunkPages=Math.max(3,Math.min(18,Math.round(pagesPerHour*(Math.max(20,targetMinutes)/60))));
  const chunks=[];
  for(let from=start;from<=item.page_end;from+=chunkPages){
    const to=Math.min(item.page_end,from+chunkPages-1);
    const pages=to-from+1;
    chunks.push({...item,chunk_key:`p${from}-${to}`,page_from:from,page_to:to,pages,planned_minutes:Math.max(15,Math.round((pages/pagesPerHour)*60))});
  }
  return chunks;
}
...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  const [onboarding,metrics,bp,readingLog,unavailability,learningProfile,studyTime]=await Promise.all([
  ...
  const catalogByKey=new Map(catalogRows.rows.map(row=>[row.bibliography_key+"|"+row.section_key,row]));
  const allReading=weighted.flatMap(s=>{
    const canonical=flattenBibliography(s.slug);
    const canonicalKeys=new Set(canonical.map(item=>item.bibliography_key+"|"+item.section_key));
    const merged=canonical.map(item=>({...item,...(catalogByKey.get(item.bibliography_key+"|"+item.section_key)||{})}));
    const dbOnly=catalogRows.rows.filter(row=>row.subject_slug===s.slug&&!canonicalKeys.has(row.bibliography_key+"|"+row.section_key));
    return [...merged,...dbOnly].map(item=>({...item,weight:s.weight,progress:progressMap.get(item.bibliography_key+"|"+item.section_key)||null}));
  });
  ...
  const targetReadingBlockMinutes=Math.max(25,Math.min(60,Math.round(readingMinutesTarget/2)));
  const pendingReading=pendingSections.flatMap(x=>readingChunks(x,x.progress?.pages_completed||0,targetReadingBlockMinutes));
  ...
  for(const day of dayObjects){
    ...
    if(phase.key!=="heavy_review"&&pendingReading.length){
      let readingBudget=readingMinutesTarget;
      let guard=0;
      while(readingBudget>=20&&guard<4&&pendingReading.length){
        const item=pendingReading[readingIndex%pendingReading.length];
        const plannedMinutes=Number(item.planned_minutes||45);
        if(guard>0&&plannedMinutes>readingBudget)break;
        day.tasks.push({
          ...
          description:item.pages?`${item.chapter} · páginas ${item.page_from}–${item.page_to}`:`${item.chapter||item.section} · recorte exato da bibliografia`,
          chapter:item.chapter,page_from:item.page_from,page_to:item.page_to,pages:item.pages,
          minutes:plannedMinutes,estimate_minutes:plannedMinutes,
          bibliography_key:item.bibliography_key,section_key:item.section_key,href:"/plano-de-estudos"
        });
        readingBudget-=plannedMinutes;
        readingIndex++;
        guard++;
      }
    }
    ...
    if(day.isoDay===studyDays[studyDays.length-1]){
      day.tasks.push({
        ...
        href:"/simulado?plan_date="+encodeURIComponent(day.iso)+"&task_key="+encodeURIComponent("weekly-exam|"+day.iso)
      });
    }
  }
  ...
  for(const day of dayObjects){
    day.tasks=day.tasks.map(task=>{
      ...
      const taskMinutes=Number(task.minutes||task.estimate_minutes||estimateTaskMinutes(task));
      return{...task,status:readingDone?"done":savedTask?.status||"pending",completed_at:readingDone?bpItem?.completed_at:savedTask?.completed_at||null,minutes:taskMinutes,estimate_minutes:taskMinutes,reason};
    });
  }
  ...
  const coverageScore=(metrics.subjects.reduce((sum,s)=>sum+Math.min(1,Number(s.questions||0)/100),0)/SUBJECTS.length)*100;
