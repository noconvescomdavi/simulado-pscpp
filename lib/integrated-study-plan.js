...
const DAY = 86400000;
const FIRST_PASS_DEADLINE = "2027-08-01"; // 3 meses antes da prova: toda bibliografia deve ter sido lida ao menos 1x.
...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  ...
  const today=new Date(), exam=new Date(EXAM_DATE+"T00:00:00-03:00"), firstPassDeadline=new Date(FIRST_PASS_DEADLINE+"T00:00:00-03:00");
  ...
  const readingDaysLeft=Math.max(1,Math.ceil((firstPassDeadline-today)/DAY/7)*activeStudyDaysPerWeek);
  ...
  for(const day of dayObjects){
    ...
    const fixationParams=readingTask
      ? "&modo=fixacao&bibliografia="+encodeURIComponent(readingTask.bibliography_key)+"&secao="+encodeURIComponent(readingTask.section_key)+"&capitulo="+encodeURIComponent(readingTask.chapter||"")
      : "";
    day.tasks.push({
      ...
      title:readingTask?targetQuestions+" questões de fixação":targetQuestions+" questões — "+subject.label,
      ...
      fixation:readingTask?{bibliography_key:readingTask.bibliography_key,section_key:readingTask.section_key,chapter:readingTask.chapter,page_from:readingTask.page_from,page_to:readingTask.page_to}:null,
      href:"/conteudos/banco-de-questoes?materia="+subject.slug+fixationParams
    });
  ...
  return{
    needs_onboarding:false,exam_date:EXAM_DATE,first_pass_deadline:FIRST_PASS_DEADLINE,days_left:daysLeft,weeks_left:weeksLeft,phase,
    first_pass:{deadline:FIRST_PASS_DEADLINE,pending_sections:pendingSections.length,known_pages_remaining:knownPagesRemaining,pages_per_reading_day:pagesPerReadingDay,on_track:today<=firstPassDeadline},
...
export async function setBibliographyStatus(userId,input={}){
  ...
  const r=await query(
    ...
    [userId,String(input.bibliography_key||"").slice(0,180),String(input.subject_slug||"").slice(0,120),String(input.section_key||"").slice(0,180),status,range,pageTo,pages]
  );
  return r.rows[0];
}
