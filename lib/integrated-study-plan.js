...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  ...
  const pendingReading=allReading.filter(x=>x.progress?.status!=="done").flatMap(x=>readingChunks(x,x.progress?.pages_completed||0));
  ...
  for(const day of dayObjects){
    ...
    day.tasks.push({
      ...
      description:"Revise erros e tópicos vencidos antes de avançar.",href:"/revisao-inteligente"
    });
