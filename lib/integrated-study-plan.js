// STUDY_PLAN_ENGINE_VERSION: v4.2-coverage-fixation
import { query } from "./db";
import { getUserMetrics } from "./metrics";
import { SUBJECTS } from "./subjects";
import { BIBLIOGRAPHY, EXAM_DATE, bibliographyUnits } from "../data/study/bibliography";
import { loadStudyPlanSnapshot } from "./study-plan-snapshot";
import {getLearningProfile,getStudyTimeSummary,explainRecommendation,readinessFromProfile} from "./learning-engine";
import {getStudentPreferences} from "./student-preferences";
...
function phaseFor(daysLeft,today=null){
  if(today&&today>=new Date("2027-08-02T00:00:00-03:00"))return{key:"heavy_review",label:"Revisão pesada",readingShare:0,questionsShare:.70};
...
function readingLocation(item={}){
  const chapter=String(item.chapter||item.section||"").trim();
  const pages=Number.isInteger(item.page_start)&&Number.isInteger(item.page_end)?`páginas ${item.page_start}–${item.page_end}`:null;
  return [chapter,pages].filter(Boolean).join(" · ");
}
...
function subjectWeight(metric,onboarding,studied,mastery={}){
  const questions=Math.max(0,Number(metric.questions||0)),accuracy=Number(metric.accuracy||0);
  const selfConfidence=Number(onboarding.confidence_by_subject?.[metric.slug]||0);
  const evidenceConfidence=Number(mastery.confidence_score||0);
  const unmeasured=questions===0;
  const weakness=unmeasured?0:Math.max(0,82-accuracy);
  const sampleNeed=unmeasured?38:Math.max(0,42-evidenceConfidence*.42);
  const coverage=Math.max(0,30-Math.min(30,questions/4));
  const selfAssessment=selfConfidence?Math.max(0,5-selfConfidence)*4:8;
  const masteryPenalty=unmeasured?0:Math.max(0,70-Number(mastery.mastery_score||0));
  return Math.max(8,weakness+sampleNeed+coverage+selfAssessment+masteryPenalty+(studied.includes(metric.slug)?-6:6));
}

function estimateTaskMinutes(task={}){
  const meta=task.metadata&&typeof task.metadata==="object"?task.metadata:task;
  const type=task.task_type||task.type;
  if(type==="reading"||type==="rereading"){
    const pages=Number(meta.pages||((meta.page_from&&meta.page_to)?Number(meta.page_to)-Number(meta.page_from)+1:0));
    return pages>0?Math.max(15,Math.round(pages*5)):45;
  }
  if(type==="questions"){
    const target=Number(meta.target_questions||0);
    return target>0?Math.max(20,Math.round(target*1.7)):35;
  }
  if(type==="simulado")return 240;
  if(type==="review")return 20;
  return 30;
}
...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  const [onboarding,metrics,bp,readingLog,unavailability,learningProfile,studyTime,preferences]=await Promise.all([
    ...
    query("select plan_date,reason,note from student_plan_unavailability where user_id=$1",[userId]).catch(()=>({rows:[]})),
    getLearningProfile(userId).catch(()=>({overall_mastery:0,subjects:[],weakest_topics:[]})),
    getStudyTimeSummary(userId).catch(()=>({today_minutes:0,week_minutes:0,month_minutes:0,week_sessions:0})),
    getStudentPreferences(userId).catch(()=>({session_minutes:45,study_mode:"balanced",dashboard_density:"standard",smart_nudges:true,prioritize_due_reviews:true}))
  ]);
  if(!onboarding?.completed_at)return{needs_onboarding:true};

  const now=new Date(),todayKey=isoDate(now),today=new Date(todayKey+"T12:00:00-03:00"), exam=new Date(EXAM_DATE+"T00:00:00-03:00"), firstPassDeadline=new Date(FIRST_PASS_DEADLINE+"T00:00:00-03:00"), internalTarget=new Date(INTERNAL_FIRST_PASS_TARGET+"T00:00:00-03:00");
  const unavailableMap=new Map((unavailability.rows||[]).map(row=>{
    const date=row.plan_date instanceof Date?row.plan_date.toISOString().slice(0,10):String(row.plan_date).slice(0,10);
    return [date,{reason:row.reason||null,note:row.note||null}];
  }));
  const daysLeft=Math.max(0,Math.ceil((exam-today)/DAY)), weeksLeft=Math.max(1,Math.ceil(daysLeft/7));
  const phase=phaseFor(daysLeft,today), weekStart=new Date(mondayOf(today).getTime()+Number(weekOffset||0)*7*DAY), weekEnd=new Date(weekStart.getTime()+6*DAY);
  const progressMap=new Map(bp.rows.map(r=>[r.bibliography_key+"|"+r.section_key,r]));
  const studied=onboarding.studied_subjects||[];
  const masteryBySubject=new Map((learningProfile.subjects||[]).map(x=>[x.slug,x]));
  const weighted=metrics.subjects.map(m=>{
    const mastery=masteryBySubject.get(m.slug);
    const questions=Number(m.questions||0);
    return {
      ...m,
      mastery_score:questions>0?Number(mastery?.mastery_score||0):null,
      confidence_score:questions>0?Number(mastery?.confidence_score||0):0,
      evidence_state:questions===0?"unmeasured":Number(mastery?.confidence_score||0)<50?"low_sample":Number(mastery?.confidence_score||0)<80?"developing_sample":"reliable_sample",
      critical_topics:Number(mastery?.critical_topics||0),
      weight:subjectWeight(m,onboarding,studied,mastery||{})
    };
  ...
  const studyDays=[...new Set([...(onboarding.study_days?.length?onboarding.study_days:[1,2,3,4,5,6]),7])].sort((a,b)=>a-b);
  ...
  const dayObjects=Array.from({length:7},(_,i)=>{
    ...
    return{date,iso,isoDay,active:studyDays.includes(isoDay),unavailable:unavailableMap.get(iso)||null,tasks:[]};
  });
  ...
  for(const day of dayObjects){
    ...
    const modeQuestionShare=preferences.study_mode==="questions"?Math.min(.55,phase.questionsShare+.15):preferences.study_mode==="reading"?Math.max(.15,phase.questionsShare-.10):preferences.study_mode==="review"?Math.max(.15,phase.questionsShare-.05):phase.questionsShare;
    const questionMinutes=Math.max(20,Math.round(dailyMinutes*modeQuestionShare));
    ...
    if(pendingReading.length){
      ...
      day.tasks.push({
        ...
        description:item.pages?`${item.chapter||item.section} · páginas ${item.page_from}–${item.page_to}`:readingLocation(item)||"Leitura da seção indicada",
    ...
    const fixationTaskKey="questions|"+subject.slug+"|"+day.iso+(readingTask?"|"+readingTask.bibliography_key+"|"+readingTask.section_key:"");
    const fixationParams=readingTask
      ? "?materia="+encodeURIComponent(subject.slug)+"&bibliografia="+encodeURIComponent(readingTask.bibliography_key)+"&secao="+encodeURIComponent(readingTask.section_key)+"&capitulo="+encodeURIComponent(readingTask.chapter||"")+"&publicacao="+encodeURIComponent(readingTask.title||"")+"&plan_date="+encodeURIComponent(day.iso)+"&task_key="+encodeURIComponent(fixationTaskKey)
      : "";
    day.tasks.push({
      key:fixationTaskKey,
    ...
    day.tasks.push({
      ...
      title:"Revisão",
      description:"Revise as questões erradas: gabarito comentado e referência da obra, capítulo/seção e página quando houver referência validada.",
      review_window:"due_errors",
      recurring_errors_priority:true,
      href:"/conteudos/caderno-de-erros?modo=revisao&plan_date="+encodeURIComponent(day.iso)
    });

    if(day.isoDay===7){
      day.tasks.push({
        key:"weekly-exam|"+day.iso,type:"simulado",subject:weighted[0]?.slug||SUBJECTS[0].slug,
        title:"Simulado PSCPP",
        description:"Emita o Simulado PSCPP semanal de 100 questões conforme a matriz oficial da plataforma.",
        href:"/simulado?novo=1&plan_date="+encodeURIComponent(day.iso)+"&task_key="+encodeURIComponent("weekly-exam|"+day.iso)
      });
    }
  }
  ...
  // GET/render é somente leitura. Se a semana já foi congelada, usa exatamente
  // as tarefas do snapshot imutável; caso contrário exibe a geração em memória.
  const frozen=await loadStudyPlanSnapshot(userId,minDate).catch(()=>null);
  if(frozen?.tasks?.length){
    const byDate=new Map();
    for(const row of frozen.tasks){
      const date=row.plan_date instanceof Date?row.plan_date.toISOString().slice(0,10):String(row.plan_date).slice(0,10);
      ...
      byDate.get(date).push({
        ...
        key:row.task_key,
        type:row.task_type,
        subject:row.subject_slug||payload.subject||null,
        estimate_minutes:payload.estimate_minutes||estimateTaskMinutes({...payload,type:row.task_type})
      });
    }
    for(const day of dayObjects)day.tasks=byDate.get(day.iso)||[];
  }
  ...
  for(const day of dayObjects){
    day.tasks=day.tasks.map(task=>{
      ...
      const subjectMetric=weighted.find(s=>s.slug===task.subject)||{};
      const reason=explainRecommendation({
        subject:task.subject,
        taskType:task.type,
        mastery:subjectMetric.mastery_score,
        accuracy:subjectMetric.accuracy,
        errors:subjectMetric.errors
      });
      return{...task,status:readingDone?"done":savedTask?.status||"pending",completed_at:readingDone?bpItem?.completed_at:savedTask?.completed_at||null,estimate_minutes:estimateTaskMinutes(task),reason};
    });
  }

  // Histórico, aderência e backlog: o que ficou para trás nunca some.
  const todayIso=isoDate(today);
  const history=await query(
    "select task_key,plan_date,task_type,subject_slug,status,completed_at,metadata from student_plan_task_progress where user_id=$1 and plan_date < $2::date order by plan_date desc,updated_at desc",
    [userId,todayIso]
  ).catch(()=>({rows:[]}));
  const historical=history.rows.map(row=>({
    ...row,
    plan_date:row.plan_date instanceof Date?row.plan_date.toISOString().slice(0,10):String(row.plan_date).slice(0,10),
    metadata:row.metadata&&typeof row.metadata==="object"?row.metadata:{}
  }));
  const thirtyDaysAgo=new Date(today.getTime()-30*DAY);
  const adherenceWindow=historical.filter(x=>
    new Date(x.plan_date+"T12:00:00-03:00")>=thirtyDaysAgo&&!unavailableMap.has(x.plan_date)
  );
  const pastPlanned=adherenceWindow.length;
  const pastDone=adherenceWindow.filter(x=>x.status==="done").length;
  const adherencePercent=pastPlanned?Math.round((pastDone/pastPlanned)*100):100;
  const backlogRows=historical.filter(x=>x.status==="pending");
  const backlog=backlogRows.map(row=>({
    key:row.task_key,
    type:row.task_type,
    subject:row.subject_slug,
    status:"pending",
    source_plan_date:row.plan_date,
    title:row.metadata.title||"Tarefa pendente",
    description:row.metadata.description||"Meta não concluída na data original.",
    href:row.metadata.href||"/plano-de-estudos",
    bibliography_key:row.metadata.bibliography_key||null,
    section_key:row.metadata.section_key||null,
    page_from:row.metadata.page_from||null,
    page_to:row.metadata.page_to||null,
    pages:row.metadata.pages||null,
    target_questions:row.metadata.target_questions||null,
    fixation:row.metadata.fixation||null,
    estimate_minutes:estimateTaskMinutes(row)
  }));
  const backlogMinutes=backlog.reduce((sum,t)=>sum+t.estimate_minutes,0);

  // Reprogramação por capacidade: tenta recuperar no máximo uma pendência por
  // dia e somente quando ela cabe no orçamento diário configurado pelo aluno.
  if(Number(weekOffset||0)>=0&&backlog.length){
    const slots=dayObjects.filter(day=>day.active&&!day.unavailable&&day.iso>=todayIso&&day.date<=exam);
    const available=slots.map(day=>({
      day,
      load:(day.tasks||[]).reduce((sum,t)=>sum+estimateTaskMinutes(t),0),
      used:false
    }));
    for(const task of backlog.slice().reverse()){
      const slot=available.find(x=>!x.used&&(x.load+task.estimate_minutes)<=dailyMinutes);
      if(!slot)continue;
      slot.used=true;
      slot.load+=task.estimate_minutes;
      const subjectMetric=weighted.find(s=>s.slug===task.subject)||{};
      slot.day.tasks.unshift({
        ...task,
        reprogrammed:true,
        display_key:"backlog|"+task.source_plan_date+"|"+task.key,
        reason:explainRecommendation({
          subject:task.subject,taskType:task.type,mastery:subjectMetric.mastery_score,
          accuracy:subjectMetric.accuracy,errors:subjectMetric.errors,reprogrammed:true
        }),
        description:"Reprogramada de "+task.source_plan_date.split("-").reverse().join("/")+" · "+task.description
      });
    }
  }

  const recoveryCapacity=dayObjects
    .filter(day=>day.active&&!day.unavailable&&day.iso>=todayIso)
    .reduce((sum,day)=>{
      const load=(day.tasks||[]).filter(t=>!t.reprogrammed).reduce((s,t)=>s+estimateTaskMinutes(t),0);
      return sum+Math.max(0,dailyMinutes-load);
    },0);
  const scheduleHealth=backlog.length===0?"on_track":backlogMinutes<=Math.max(30,recoveryCapacity)?"attention":"behind";

  // Readiness is derived after backlog and recovery-capacity calculations.
  const readiness=readinessFromProfile(learningProfile);

  return{
    needs_onboarding:false,exam_date:EXAM_DATE,first_pass_deadline:FIRST_PASS_DEADLINE,internal_first_pass_target:INTERNAL_FIRST_PASS_TARGET,days_left:daysLeft,weeks_left:weeksLeft,phase,preferences,
    first_pass:{
    ...
    week:{start:minDate,end:maxDate,offset:Number(weekOffset||0),days:dayObjects,snapshot_id:frozen?.snapshot?.id||null,snapshot_version:frozen?.snapshot?.algorithm_version||null},
    tracking:{
      adherence_percent:adherencePercent,
      past_planned:pastPlanned,
      past_done:pastDone,
      ...
      recovery_capacity_minutes:recoveryCapacity,
      unavailable_days:[...unavailableMap.entries()].map(([plan_date,value])=>({plan_date,...value})),
      study_time:studyTime,
      overall_mastery:Number(learningProfile.overall_mastery||0),
      weakest_topics:(learningProfile.weakest_topics||[]).slice(0,5),
      backlog:backlog.slice(0,12)
    },
    ...
    readiness,
    phases:[
      {label:"Primeira passagem",from:"Cadastro",to:"30/07/2027",focus:"Ler 100% da bibliografia exigida pelo menos uma vez, por capítulos/seções e páginas"},
      {label:"Revisão pesada",from:"31/07/2027",to:"31/10/2027",focus:"Questões, simulados, caderno de erros, revisão espaçada e releitura seletiva dos tópicos de menor pontuação"}
    ]
  };
}
