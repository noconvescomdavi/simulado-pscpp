...
export async function getIntegratedStudyPlan(userId,weekOffset=0){
  ...
  // Reprogramação suave: no máximo uma pendência por dia de estudo futuro.
  // A tarefa continua pertencendo à data original; concluir a cópia reprogramada
  // dá baixa no registro original e preserva a trilha histórica.
  if(Number(weekOffset||0)>=0&&backlog.length){
    const slots=dayObjects.filter(day=>day.active&&day.iso>=todayIso&&day.date<=exam);
    backlog.slice().reverse().slice(0,slots.length).forEach((task,index)=>{
  ...
  const scheduleHealth=backlog.length===0?"on_track":backlog.length<=3?"attention":"behind";
