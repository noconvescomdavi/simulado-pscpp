...
export function buildStudentInsights({plan,learning,time,reviews,consistency,metrics}={}){
  if(plan?.needs_onboarding)return{needs_onboarding:true,insights:[]};
  learning=learning||{overall_mastery:0,subjects:[],weakest_topics:[]};
  time=time||{today_minutes:0,week_minutes:0};
  reviews=Array.isArray(reviews)?reviews:[];
  consistency=consistency||{streak:0,study_days:0};
...
export async function getStudentInsights(userId){
  ...
  return buildStudentInsights({plan,learning,time,reviews,consistency,metrics});
}